...
- [ ] **Build a SECOND integer-supported (a_1 != 0) off-line control** (gating question for M3 -> Direction 8): a Davenport-Heilbronn-type linear combination at a different conductor, or any engineered Dirichlet series with a functional equation, a_1 = 1, and a known off-line zero. Confirm M_euler < 0 on it. Until this exists, "M_euler detects RH" rests on one off-line witness (D-H).
- [ ] **Stress-test the GEOMETRIC M3 conjecture (P_comp = 0 geometrically)**: attack the claim that absolute prismatic cohomology kills composite cross-terms. If it fails, the hand-deletion of P_comp is the Euler-product assumption in disguise rather than a geometric law.
