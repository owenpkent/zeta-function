...
- [ ] **Product Surface Intersection Signature**: Geometricize the M3 trace-side positivity bound (M_euler) as a Hodge index signature on the product surface Spec(Z) x Spec(Z), via Bhatt-Lurie's WCart or Connes-Consani's N^2-hat.
  ...
  - [ ] **The arithmetic q-lift (the sharpened gap):** deform the mixed-volume form (or build a Weil cohomology of the C-C square) whose Delta.Gamma is the point count q+1-t, restoring the trace. Candidate carriers: (a) the q->1 un-idempotent-ization of the characteristic-1 operations; (b) the Psi(lambda)oPsi(lambda^-1)=Id_eps tangential-deformation self-intersection (Connes-Consani Thm 7.7) vs 2R's -zeta'/zeta. The (1,p) bidegree (2Q) is the locus where the lift must inject t.
