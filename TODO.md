...
- [ ] **Milestone M2.6 (immediate next): derive + validate the general-mu archimedean Bombieri kernel** (mu=1 for D-H, mu in {0,1} for Epstein; the current mu>0 kernel is mis-normalized, D-H residual 5.58). Unblocks the four-way RH-vs-Euler verdict: does min eig(M) > 0 for zeta and Epstein-principal (RH) and < 0 for D-H (RH-fails)? If yes, the certificate separates RH-failure from non-Euler-ness (defeats the #20 trap). See 08A §4.
