...
- [x] **Milestone M3 (must be ANALYTIC, per M2.6): prove the archimedean-dominates-prime domination engaging the EXACT off-line structure** (the Euler product / Gamma-factor cancellation that distinguishes zeta from D-H below the marginal-positivity floor). DONE (e3m_analytic_domination.py). Stealth window broken by isolating composite obstruction (P_comp).
