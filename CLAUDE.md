...
- **The xi arc (the SP5 ladder in contact with a live external program, 2026-08-19/21)**: LEARNINGS #179-#190. The assembled SP-object's Weil-form ground state, measured across window scales with certificates, against the Connes-Consani-Moscovici / Suzuki limit conjecture (their (1.2)): the state IS $\Xi$ at $a = 1$, collapses monotonically after (certified, no turnaround through $a = 4$), and CCM's kernel-groundstate "educated guess" (their self-named main remaining obstacle) decays measurably past their evidence range while their kernel is exactly $\Xi$ throughout. Library: `references/10_weil_positivity/`; packaging: PUBLICATIONS P12; entry points: [`e2av_kernel_proximity.md`](experiments/arithmetic_geometric/e2av_kernel_proximity.md) and rung 7 of the interface doc.
