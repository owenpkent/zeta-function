...
- **The Generative Engine (the generate half)**: [`docs/03_research/generative_engine.md`](docs/03_research/generative_engine.md). Design (not built) for the organ that PROPOSES: a move-library + transfer-search (bridges, not atoms) + quality-diversity over failure cells, built around the value-signal blind spot (iterate on reformulations and bridges, not proofs; use the function-field shadow as the positive gradient and D-H as the negative one). The Reduction Engine disposes; this proposes.
