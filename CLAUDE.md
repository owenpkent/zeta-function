...
- **The trojan-horse ledger (how NOT to smuggle the sign, and what smuggling IS for)**: [`docs/03_research/trojan_horse_m4.md`](docs/03_research/trojan_horse_m4.md). The measured answer to "can M4 be trojan-horsed": five costumes inventoried (state/GNS, self-adjointness, one-sided Landau, convexity, logic), every one pays at the identification/uniformity joint or the carrier joint, and the tariff is consuming the Euler product plus the additive lattice at the same joint (the conservation law; D-H and Beurling are the fare-dodging detectors). The compactness costume was executed end to end (LEARNINGS #170 e1t, #171 e1u): compactness is free exactly where it is information-free, and the canonical-chain door closed by its own pre-registered exit as the THIRD proven coordinate system for M4 (variational / lattice-Hamburger / Weyl-spectral-chain, where the wall reads as uniform Christoffel growth). What trojan-horsing IS for, proven twice (#160's positivity-free pin, #171's chain): minting new coordinate systems for the same wall. Section 9 (LEARNINGS #172, e1v) closes the Christoffel costume the arc named: the germ-length blowup is now a THEOREM whose rate depends only on (gap, support, count), the tightness residual is family-blind under mandatory band equalization, and the functional is provably unable to see Q-linear independence because it is continuous while lattices are dense. Scope, established by the round's own adversary: that obstruction is strictly POINTWISE, so the surviving question is sequence-level (sum rules), and the sweep is to be SPLIT rather than cancelled.
- **The xi arc (the SP5 ladder in contact with a live external program, 2026-08-19/21)**: LEARNINGS #179-#191. The assembled SP-object's Weil-form ground state, measured across window scales with certificates, against the Connes-Consani-Moscovici / Suzuki limit conjecture (their (1.2)): the state IS $\Xi$ at $a = 1$, collapses monotonically after (certified, no turnaround through $a = 4$), CCM's kernel-groundstate "educated guess" (their self-named main remaining obstacle) decays measurably past their evidence range while their kernel is exactly $\Xi$ throughout, and the energy coda (e2aw, #191) INVERTS the bridge: the kernel's exact-vanishing identity makes the continuum valley doubly-exponentially deep ($\lg B \approx -2\pi e^{2a}/\ln 10$), undercutting every direct-minimization floor from $a = 1.5$: the collapse is a resolvable-subspace statement, (1.2) is horizon-limited, and nothing measured contradicts CCM's continuum expectation. Library: `references/10_weil_positivity/`; packaging: PUBLICATIONS P12 (draft v0.2 in `publications/weil_ground_state/`); entry points: [`e2aw_energy_gap.md`](experiments/arithmetic_geometric/e2aw_energy_gap.md) and rung 7 of the interface doc.
- **The primes thread (measurement, and what measurement cannot do)**: [`experiments/primes/PRIME_PATTERNS.md`](experiments/primes/PRIME_PATTERNS.md). Every prime to $10^{13}$ and every zero to height $5\times10^7$ (118,488,122 of them, every sign certified), computed here. Two things it settles for the main program. POSITIVE: the Chebyshev races are the zeros of Dirichlet L-functions talking, and precisely enough to PREDICT where a bias breaks (the mod-3 excursion at $6.15\times10^{12}$ was called from the zeros before the primes were counted); unlike the GUE statistics, that observable is sensitive to the zeros' REAL parts. NEGATIVE: GUE agreement is provably RH-blind (move zeros off the line, every statistic is unchanged bit for bit), and no prime-side observable can test RH at all, because any off-line zero hides above height $3\times10^{12}$ and needs $x \sim 10^{150}$ to surface. Which is why RH verification counts ZEROS, and why the axiom census (LEARNINGS #174) matters: duality + integrality + Euler buy only the trivial region. Engines: `primestream_gpu.py` (GPU sieve, bit-identical to the CPU reference), `rsz.py` (bulk Riemann-Siegel zeros).
...
│   ├── primes/                  prime-patterns thread: digits/bases, LOS bias, Chebyshev races, twins vs HL, explicit formula both
│   │                            directions, GUE statistics, RH verification, races predicted from L-zeros (PRIME_PATTERNS.md, DATASETS.md)
...
# Full regression battery: every test_*/smoke_test module, one summary table (expect "10/10 modules fully passed")
