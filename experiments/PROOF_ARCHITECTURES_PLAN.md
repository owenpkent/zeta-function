...
| Arch 1 corridor + post-corridor pivot (e1k-e1u and after; live thread) | 🔄 | **This table stops at Arch 1F by design; the live spectral thread is tracked round-by-round in [`PHASE_STATE.md`](../PHASE_STATE.md) and [`LEARNINGS.md`](LEARNINGS.md) (#154-#173), not here.** Current status as of 2026-07-31: the CCM corridor CLOSED as a proof home (#163/#164, Owen-ratified) and stays open as the program's best measurement instrument; the S4/R1 coordinate is closed at every buildable corner (#162/#169) with its classical floor machine-checked in [`S4Carrier.lean`](../lean/ZetaRH/S4Carrier.lean); the compactness arc closed as the THIRD reformulation of M4 (#170/#171); both named pivot sweeps (modular/Hecke, Christoffel) landed NEGATIVE with mapped absences (#172); the two #169 residuals are closed, one of them with a REVERSED verdict after a believed-RH-true Euler control falsified its attribution (#173). **Architecture-level status is unchanged by all of it: Arch 1 remains closed at the numerical-experiment level, and M4 (the Arch 2 Hodge-index slot, row 2A below) remains the open kernel.** |
