...
| Arch 3D.4 (T_max scaling) | ✅ | **Validates the structural prediction $n_{\rm neg}$ = # off-line $\gamma$'s in UHP up to $T_{\max}$.** Sweep $T_{\max} \in \{200, 300, 500, 800\}$: at $T_{\max} = 200$ D-H has 4 distinct off-line $\gamma$'s → 4 neg eigenvalues; at $T_{\max} = 300$ a new off-line pair appears at $\gamma \approx 240.4$ → 5 neg eigenvalues. Higher $T_{\max}$ continues to match. Selberg-class L-functions stay PSD at all $T_{\max}$. **The detector is structurally counting off-line zero pairs.** Strongest validation of the wrong-approach detector's architectural picture. |
