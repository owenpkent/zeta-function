...
| Arch 2A (Weil-proof diff table) | ✅ | [`2A_weil_proof_diff.md`](arithmetic_geometric/2A_weil_proof_diff.md): step-by-step diff between Weil's proof for curves over $\mathbb{F}_q$ (Lefschetz + Poincaré + Hodge index) and the missing structure over $\mathrm{Spec}(\mathbb{Z})$. **Conclusion: Architecture 2's obstruction is constructive, not analytic.** Each step has a partial analogue at best: no geometric Frobenius on $\mathrm{Spec}(\mathbb{Z})$, no proper compactification giving the right cohomology, no surface $\mathrm{Spec}(\mathbb{Z}) \times_{\mathbb{F}_1} \mathrm{Spec}(\mathbb{Z})$ in any rigorous sense, hence no Hodge index analogue. Connes / Deninger / $\mathbb{F}_1$ programs each address one corner; none has assembled all three pieces on a single object. |
