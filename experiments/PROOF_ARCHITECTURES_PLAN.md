...
| Arch 1D (Connes literature) | ✅ | [`1d_connes_adele_literature.md`](spectral/1d_connes_adele_literature.md), brought current 2026-06 in [`quantum_mechanics_signature_dossier.md`](spectral/quantum_mechanics_signature_dossier.md). Connes-Moscovici prolate operator dissolves the 1A density mismatch intrinsically; archimedean Weil-positivity PROVEN but K2-blind (D-H-shared); 2024 semilocal prolate (arXiv:2310.18423) injects Euler content on the Frobenius side but positivity is strategy-not-theorem (K1 risk, door ajar). Bender-Brody-Müller (2017) recorded as the canonical K1/R3.5 circular-spectral instance (Bellissard operator-existence kill). |
| Arch 1E (arithmetic input as additive potential) | ✅ | [`e1d_arithmetic_spectral_dh.py`](spectral/e1d_arithmetic_spectral_dh.py). NULL: injecting $\{\log p\}$ as an additive potential/kernel does NOT break the $\zeta$-vs-D-H symmetry (three controls: random comb ties, scrambled von-Mangoldt weights inert, D-H-built operator still prefers $\zeta$). Arithmetic must enter the GEOMETRY (per-prime $\mathbb{Q}_p$), not the dilation generator. Sharpens 1A-1C. |
| Arch 1F (CCM semilocal prolate + the metaplectic operator) | ✅ | The one live spectral thread (Connes-Consani-Moscovici 2310.18423), mapped + four operator builds (#114-#118, [`ccm_semilocal_prolate.md`](../docs/03_research/ccm_semilocal_prolate.md)). The open step factors into a separately-attackable operator construction around the M4 core; R3.5 status conditionally escaping. Three cheap OP-data surrogates all signature-blind (`e1f` non-idempotent, `e1g` reweighting-blind, `e1h` reads-moments; validated identity: the archimedean scaling operator IS Meixner-Pollaczek). The un-eliminated **metaplectic operator** then attacked at its finite-local core (`e1i`, [`e1i_metaplectic_weil_index.py`](spectral/e1i_metaplectic_weil_index.py)): exact gate $\mathrm{Tr}(F_p)=\varepsilon_p$ + the double-cover cocycle, but the finite-local quadratic sign-structure is signature-blind under the positivity compression (5/5 Arbiter body, false-negative-probed) and the scalar Weil index cancels in $g^*g$. Does NOT eliminate CCM's global semilocal $W_{\lambda,S}$ = M4; the spectral architecture is closed at the cheap/operator/finite-local level. |
| Arch 1G (e1w: Burnol literal bilinear extension audit) | ✅ | [`e1w_burnol_bilinear.py`](spectral/e1w_burnol_bilinear.py) + [`.md`](spectral/e1w_burnol_bilinear.md) + `.npz`, 24/24 full / 18/18 quick; adversary PASS, all six attacks MISSED ([`_e1w_adversary.md`](spectral/_e1w_adversary.md)). **CORRECTED at source:** Burnol's literal extension $K_a \to L_a$ (math/0203120; a sub-Hilbert space, verbatim) has signature $(2,0)$, $\kappa(L_a) = 0$ at every $a$ tested (two independent routes agreeing to $3.5\times10^{-36}$; adversary beyond-grid probes to $a = 2.5$, worst sign margin $2.7\times10^{14}$), refuting LEARNINGS #168(iii)'s candidate-tier $\kappa = 1$ ansatz. New residue dictionary $\mathrm{Res}_1 M(f) = -f(0^+)$, $\mathrm{Res}_0 M(f) = (\mathcal{F}_+f)(0^+)$, $Y_0 = -\mathcal{F}_+Y_1$. The ansatz's $(1,1)$ block survives only at the $\mathcal{F}_+$-twisted pairing (Krein completion provably non-Pontryagin, $\kappa = \infty$). Typing: FORM-SIDE/STRUCTURAL (consumes FE + interval constancy, no Euler product; makes no zeta-vs-fake claim). LEARNINGS #173. |
...
3. **Architecture 1 (spectral)**: rich numerical playground; partial machinery already from multifractal work. ✅ Complete through 1C; 1D remains literature. The post-1F operator corridor (e1j-e1w, LEARNINGS #158-#173) is tracked at finding level in LEARNINGS/PHASE_STATE; latest landed row is Arch 1G (e1w, the #168(iii) source-tier correction).
