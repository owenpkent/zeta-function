...
**Date:** 2026-08-13, deepened 2026-08-14, verification thread extended 2026-08-16/17 (LEARNINGS #175). **Modules:** `e5a_digit_patterns.py`, `e5b_twin_primes.py`, `e5c_explicit_formula.py`, `e5d_riemann_spectrum.py`, `e5e_zero_statistics.py`, `e5f_rh_verification.py`, `e5g_race_from_zeros.py`, `e5h_multichar_races.py`; engines (CPU and GPU) in `primestream.py` (segmented sieve) and `rsz.py` (Riemann-Siegel bulk zero finder); checks in `test_primes.py` (55/55). **Scale:** every number below is measured in this repo. Default pass $N = 10^8$ (seconds); deep pass $N = 10^{13}$, all **346,065,536,839** primes streamed in 8.3 hours on the box's GPU (the same pass is ~4.4 days on its CPU; see `primestream_gpu.py`).

**Predictions registered before each deep pass ran, and how they came out.** Eight for eight:

| quantity | predicted | measured |
|---|---|---|
| $\pi(10^{13})$ | 346,065,536,839 | 346,065,536,839 |
| twin pairs below $10^{12}$ | 1,870,585,220 | 1,870,585,220 |
| twin pairs below $10^{13}$ | 15,834,664,872 | 15,834,664,872 |
...
| base-10 repeat share, decade $10^{12}$ | 0.1987 | 0.1987 |
| mod-3 excursion window (from the zeros) | 6.1458 to 6.1568 e12 | episode at 6.1482 to 6.1561 e12 |

The first five rows are table lookups: published values our engine had to reproduce, and did, exactly. The last three are not. The two repeat-share rows come from fitting $a + b\log\log x/\log x$ to the decades already measured and extrapolating one further; that fit called 0.1951 for the ninth decade and then 0.1987 for the tenth, both to four decimals, before the passes that would test them ran. The last row is sharper still: the zeros of $L(s,\chi_3)$ named a window for the next mod-3 excursion two orders of magnitude past anything measured, and the $10^{13}$ pass found the episode inside it (§3b). Retrodiction is cheap; these were registered first.
...
Within the legal digits, primes equidistribute (Dirichlet 1837, made quantitative by the prime number theorem in arithmetic progressions). Measured at $10^{13}$: every share in every tested base $b \in \{3,4,10,12,30\}$ is within $3.1 \times 10^{-7}$ of $1/\varphi(b)$ (base 10: within $1.3 \times 10^{-7}$ of exactly $1/4$). There is no favored last digit in any base, asymptotically.

## 2. Why primes do not repeat their last digit (the Lemke Oliver-Soundararajan bias)

Consecutive primes avoid sharing a last digit, by a lot. Measured transition matrix at $10^{13}$, base 10 (row = this prime's digit, column = next prime's):

|      | 1 | 3 | 7 | 9 |
|------|------|------|------|------|
| **1** | 0.1998 | 0.2852 | 0.2844 | 0.2305 |
| **3** | 0.2457 | 0.1966 | 0.2732 | 0.2844 |
| **7** | 0.2535 | 0.2647 | 0.1966 | 0.2852 |
| **9** | 0.3009 | 0.2535 | 0.2457 | 0.1998 |

A memoryless process would put 0.25 everywhere. Even after 346 billion primes the diagonal sits at 0.197-0.200: a repeat is still about 21% less likely than chance (31% at $10^8$: the deficit shrinks, slowly). The bias is in every base (repeat share vs naive $1/\varphi(b)$ at $10^{13}$): base 3, 0.4602 vs 0.5; base 4, 0.4629 vs 0.5; base 10, 0.1982 vs 0.25; base 12, 0.1993 vs 0.25; base 30, 0.0708 vs 0.125 (still a 43% deficit at this depth, 65% at $10^8$). More legal digits, stronger avoidance.

The mechanism is not mystical. (i) The cheap first-order reason: after a prime ending in 1, the candidates ending in 3, 7, 9 come **before** the next candidate ending in 1; since the average gap near $10^8$ is $\log 10^8 \approx 18.4$, comparable to one decade, the in-between digits get first shot. (ii) The precise reason (Lemke Oliver-Soundararajan 2016): the Hardy-Littlewood $k$-tuple conjecture predicts the full matrix, with the diagonal deficit decaying like $\log\log x / \log x$, one of the slowest decays in analysis. Measured repeat share by decade $[10^k, 10^{k+1})$, base 10: 0.1206 ($10^3$), 0.1422, 0.1555, 0.1643, 0.1730, 0.1802, 0.1861, 0.1910, 0.1951, 0.1987 ($10^{12}$). Ten decades of crawl and it is still a fifth short of 0.25; at any scale a machine will ever compute, it visibly has not arrived. The fit $a + b\log\log x/\log x$ over the first eight decades predicted the ninth to four decimals, and the SAME fit then called the tenth (0.1987) to four decimals as well, before the $10^{13}$ pass ran. Intercept $a = 0.2564$ against the theoretical limit $0.25$. So: not a law, a bias; the asymptotic limit is uniform $1/\varphi(b)$, approached logarithmically slowly.
...
**Mod 4** (base-4 last digit 3 vs 1), measured to $10^{13}$: the 3-side leads almost always, peaking at +195,952. The 1-side takes the lead only in narrow windows, first at exactly $x = 26{,}861$ (Leech 1957, reproduced by the stream) and deepest at $x = 9.32 \times 10^{12}$, where it gets **35,315** primes ahead before the 3-side recovers (the $10^{12}$ pass had seen only $-2{,}719$; the excursions keep growing like $\sqrt x$). Our log-density estimate of "3 ahead" is 0.9987, against the Rubinstein-Sarnak value 0.9959 (1994, under GRH plus linear independence of zeros).
...
The shape of the excursion is the interesting part. It does not merely graze zero: the lead plunges to $-1{,}538$ at $x = 609{,}224{,}663{,}413$, some 240 million past the crossing, then repairs itself. And the scales are exactly what the theory orders. At the flip, $\sqrt{x}/\log x = 28{,}759$, which is simultaneously the size of the systematic bias (the prime squares $p^2 \equiv 1$ that class 1 is "credited" with, $\tfrac12\pi(\sqrt x) = 31{,}296$) and the size of the fluctuation the L-function zeros supply. Drift and noise are the same order, which is why the bias is persistent, why it is not permanent, and why Littlewood could prove the sign changes infinitely often. Measured against that scale, the peak lead is $1.64\sqrt{x}/\log x$ and this first dip only $0.053$ of it. In relative terms the race lives in the eighth digit: 1,538 out of 23 billion is an imbalance of $6.7\times10^{-8}$.

And the failure is not a single crossing. The excursion catalogue records **every** maximal negative run, and the picture up close is far more violent than "crosses zero, recovers": the $6.09\times10^{11}$ failure is **10,408 separate dips** falling into two episodes (6.0898 to 6.0934 e11, negative 42% of that window, and a second at 6.1070 to 6.1097 e11, negative 63% of it), and the $6.15\times10^{12}$ event the zeros predicted is another **31,854**. Across the whole $10^{13}$ pass the mod-3 lead goes negative in 42,262 separate runs, and the deepest point is no longer the first flip at all: it is $-3{,}600$ at $x = 6{,}151{,}545{,}058{,}303$, inside the predicted window. Neither `first_cross` nor `min_lead` alone would have shown any of this.
...
The twin prime conjecture (infinitely many $p$ with $p + 2$ prime) is open. What is not open is how twins are **distributed**, conjecturally to extraordinary accuracy. Hardy-Littlewood: $\pi_2(x) \sim 2 C_2 \int_2^x dt/\log^2 t$ with $C_2 = \prod_{p > 2} (1 - (p-1)^{-2}) = 0.6601618\ldots$ (we compute $C_2$ to $10^{-8}$ from a sieve). Measured at $10^{13}$:

| constellation | measured | HL predicted | ratio |
|---|---|---|---|
| $(p, p+2)$ twins | 15,834,664,872 | 15,834,598,398 | 1.000004 |
| $(p, p+4)$ cousins | 15,834,656,003 | 15,834,598,398 | 1.000004 |
| $(p, p+6)$ sexy | 31,669,317,884 | 31,669,196,796 | 1.000004 |
| $(p, p{+}2, p{+}6)$ triplets | 1,189,795,268 | | |
| $(p, p{+}4, p{+}6)$ triplets | 1,189,826,966 | | |
| $(p, p{+}2, p{+}6, p{+}8)$ quadruplets | 60,070,590 | | |

Three parameter-free predictions, three hits at the $3 \times 10^{-5}$ level, including that $d = 6$ pairs are exactly **twice** as common as twins (singular series factor $(3-1)/(3-2) = 2$; measured ratio 1.999999, six digits). The convergence is visible decade by decade: the twin ratio walks 0.957 ($10^4$) $\to$ 0.9904 ($10^6$) $\to$ 0.99987 ($10^8$) $\to$ 1.000032 ($10^{11}$) $\to$ 1.000014 ($10^{12}$) $\to$ 1.000004 ($10^{13}$).

Two things worth noticing in that table. **Cousins tie twins to ten digits**: 15,834,656,003 against 15,834,664,872, a difference of 8,869 in 15.8 billion (6 parts in $10^7$), because their singular series is not approximately equal but *identical*. The two admissible triplet shapes tie likewise (1,189,795,268 vs 1,189,826,966). Nothing in the primes distinguishes $\{0,2\}$ from $\{0,4\}$, or $\{0,2,6\}$ from $\{0,4,6\}$: only the local obstructions matter, and those agree. The same series makes 6 the most common gap between consecutive primes from $x \approx 10^3$ on (measured champion in every decade; at $10^{13}$ the leaderboard runs 6 (2.93e10) > 12 > 18 > 10 > **30** > **24**, with 30 and 24 having overtaken 4 and 2 since $10^{12}$: the primorial-flavored gaps visibly climbing; conjectured to hand over to 30 near $10^{35}$, then 210: Odlyzko-Rubinstein-Wolf 1999).

The two famous sums, measured: $\sum_{p \le 10^{13}} 1/p = 3.660479$ vs Mertens $\log\log x + M = 3.660479$ (agreement to every printed digit; divergent, glacially); Brun's twin sum partial $\sum (1/p + 1/(p+2)) = 1.813944$ at $10^{13}$, against the extrapolated full value $B_2 \approx 1.902161$ (convergent: Brun 1919, which is why the twin conjecture cannot be settled by the series alone).

**A new one from the adjacent-gap accumulator: gaps remember their predecessor, and one transition is outright forbidden.** Comparing the joint law of consecutive gaps $(g_n, g_{n+1})$ against the independent product at $10^{13}$: a gap of 2 followed by another gap of 2 has observed/independent ratio **exactly 0.0000**, because $p, p+2, p+4$ always contains a multiple of 3, so no twin gap can ever be immediately followed by another. A gap of 2 followed by 4 is **1.642x** more likely than independence (that is the admissible triplet $\{0,2,6\}$ asserting itself), and 6 followed by 6 is suppressed to 0.819x. The consecutive-gap sequence is not a renewal process: the same admissibility bookkeeping that prices the constellations also prices the transitions between gaps. Current infinitude state of the art: some gap $\le 246$ occurs infinitely often (Zhang 2013; Maynard-Tao; Polymath8b), 6 under the generalized Elliott-Halberstam conjecture.
...
> Every one of the **118,488,122** zeros of $\zeta$ with $0 < \operatorname{Im}\rho \le 5\times10^7$ is simple and lies exactly on the critical line, **every sign certified**, and (by the Turing closure below) there are no others.

99,464,739 Gram blocks, Gram's law failing on 15.57% of them (longest block 8), 34 boundary merges, zero unresolved blocks, **114** signs escalated to exact arithmetic, 15h28m on 8 workers with extended-precision phases and the $C_1$ correction (2026-08-17). Turing closes it: $S(g_m) \le 0.1237 < 1$ at the top, so the integer $S(g_m) = 0$.

The escalation count is the number to look at. At $2\times10^7$ with float64 phases and $C_0$ only, 1,060 points in 44.5 million needed exact arithmetic; at $5\times10^7$ with extended phases and $C_1$, **114 in 118.5 million**, a 25-fold drop in rate at 2.5x the height. That is the certified bound at the top of the range falling from $7.3\times10^{-5}$ to $3.6\times10^{-8}$, a factor of 2050.

Two earlier passes of this pipeline are what the diagnostics below were written against: $10^7$ ($21{,}136{,}125$ zeros, 14.85% Gram exceptions, one boundary merge, 1h54m, uncertified) and $2\times10^7$ ($44{,}478{,}605$ zeros, 15.18%, 6 merges, 1,060 escalations, 3h52m on 6 workers, certified). Getting to $5\times10^7$ took the three levers described further down.
...
**The wall past $10^7$ is the arithmetic, and it moves (2026-08-16).** Read the two terms of $\varepsilon(t)$ against each other: truncation falls like $t^{-3/4}$ while rounding rises like $t^{5/4}\log t$, so they cross at $t \approx 3.0\times10^6$ (measured by `rsz.rounding_crossover`) and by $10^8$ the bound is rounding-dominated by a factor of 1400. Going higher therefore needs better *phase arithmetic*, not more correction terms. That is now built: `zed(..., prec="ld")` forms $\theta(t) - t\log n$ and reduces it mod $2\pi$ in x86 80-bit extended precision, whose unit roundoff $2^{-64}$ is 2048 times smaller, and only then drops to float64 for the cosine. The crossover moves from $3.0\times10^6$ to $1.19\times10^8$, a factor of 40 in certified height, and the bound at $t = 10^8$ falls from $1.8\times10^{-4}$ to $2.2\times10^{-7}$. Checked against `mpmath.siegelz`, not just derived: at $10^8$ the extended path errs by $1.6\times10^{-8}$ where float64 errs by $2.8\times10^{-7}$, and both stay inside their own bounds at every height sampled. It shows up where it counts, in escalations to exact arithmetic on a 16,000-index band at the top of the range: 2 at $t \approx 10^7$ and 5 at $3\times10^7$ for float64, **0 at both** for the extended path, at 1.8x the cost per point. The float64 path is untouched and remains the default, so every number recorded above reproduces bit-for-bit (checked by direct comparison against the pre-change routine). Platform note: extended precision is an x86 feature, so on a machine whose `longdouble` is really float64 the option refuses rather than claiming a bound it cannot honour, and `best_prec()` picks correctly.

**And the walk parallelizes exactly (2026-08-16).** The Gram walk looks sequential because each window starts where the last ended, but that dependency dissolves at any Gram point where Gram's law holds: a band bounded by two such points contains whole Gram blocks, so its count is exact on its own and the bands' counts add. `verify_parallel` cuts $B = 3 \times$ workers bands at seams found by `good_gram_at_or_after`, weighted as $n_i = n_{\text{end}}(i/B)^{2/3}$ because the Riemann-Siegel cost density in the index goes like $\sqrt{g_n}$, and hands them to a process pool. This is not an approximation: serial and parallel agree exactly on the found count, the expected count and the verdict, and the returned `tiled` flag asserts that consecutive bands meet. Measured on 6 workers with the box otherwise busy: 3.7x.

**Then $C_1$, which also closed a hole in the bound (2026-08-17).** With rounding no longer dominant, the $C_0$-only truncation $0.127\,t^{-3/4}$ became the binding term, so `zed(..., k=1)` adds the $C_1$ correction and drops it to $0.053\,t^{-5/4}$. Both coefficients are carried as degree-26 Chebyshev fits, audited by a `_regen_correction_coeffs()` hook the test suite calls and compares. Worth recording how the constant was pinned: the textbook $C_1 = -\Psi'''/(2^5\cdot3\pi^2)$ is stated for the variable $u = p - 1/2$ while this code uses $z = 2u$, so the right coefficient here is $-\Psi'''(z)/(12\pi^2)$, and that factor of 8 was caught by *measurement*, not by reading: the residual left after the $C_0$ term, rescaled by $a^{3/2}$, came out 8.0000x the naive version (median 7.99912 over 800 samples). After the fix the error falls 295x to 4788x against `mpmath.siegelz` over $t = 10^5$ to $10^9$, always inside its own bound, and $k=0$ vs $k=1$ certified verifications agree exactly on the count.

Implementing it exposed a genuine gap in the *bound*. The $C_0$ quotient $\cos(2\pi(p^2-p-1/16))/\cos(2\pi p)$ is $0/0$ at the removable zeros $p = 1/4, 3/4$: finite in value, but computed as a ratio of two vanishing quantities, so it loses most of its digits nearby. Measured error up to $8.3\times10^{-2}$, five orders above typical, and the old rounding bound accounted for the main sum only and did not cover it at all. `rs_rounding_bound` now carries a pointwise $6u/|\cos(2\pi p)|$ term, so those points escalate to exact arithmetic instead of being trusted, with a regression test pinning it. The completed $2\times10^7$ run is unaffected: a Gram point would have had to land within $\sim10^{-13}$ of $p = 1/4$ for the old bound to be violated, expected count $\sim2\times10^{-5}$ over 44.5 million points.

**The three levers compose, and the run that used them is the result quoted above.** At the top of the $5\times10^7$ range the certified bound is $3.55\times10^{-8}$, against $7.28\times10^{-5}$ at the float64 / $C_0$ default. Escalations concentrate at *low* height, where $0.127\,t^{-3/4}$ is still large and everything below $t = 200$ is escalated by construction because Gabcke's bound does not apply there.
...
python -m experiments.primes.e5f_rh_verification 1e7               # verify RH to height 10^7 (~2 h serial)
python -m experiments.primes.e5f_rh_verification 1e6 certified     # ... with every sign certified
python -m experiments.primes.e5f_rh_verification 5e7 certified workers=8 prec=auto k=1  # past 10^7: all cores, extended phases, C_1
...
python -m experiments.primes.test_primes                           # 55/55; auto-discovered by run_all_tests
```
