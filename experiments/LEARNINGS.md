...
### 43. The de Branges / Conrey-Li per-zero cross-term Q(ρ) = -Re{ξ'(ρ)ξ(1+ρ)} is a NEGATIVE coordinate and the exact CONVERSE of #42: the GLOBAL signed pairing de Branges already built DOES see the zeros, but its pointwise positivity is strictly-stronger-than-RH and FAILS sporadically even when RH holds. So the M3 signed pairing must be RH-EQUIVALENT (a sum, like λ_n), not the pointwise Hermite-Biehler cross-term.
