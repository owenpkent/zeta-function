...
## CCM semilocal prolate operator builds (e1f / e1g / e1h, 2026-06-24/25)

The one live thread from the QM survey is the Connes-Consani-Moscovici semilocal prolate operator (arXiv:2310.18423), the only QM object on the Frobenius side with an explicit positivity strategy. The dedicated map + deep-read is [`../../docs/03_research/ccm_semilocal_prolate.md`](../../docs/03_research/ccm_semilocal_prolate.md) (the open step factors into a separately-attackable operator construction; R3.5 status conditionally escaping). Three faithful operator builds attempted it, **all signature-blind by distinct mechanisms** (the cheap operator routes are exhausted; the metaplectic operator CCM deferred is the un-eliminated route):

- **[`e1f_ccm_semilocal_prolate.py`](e1f_ccm_semilocal_prolate.py)** — a multiplication-by-density surrogate. Reproduces the archimedean sign-source (the $-2$ jump + the Slepian/PSWF spectrum) but its "projection" is **not idempotent**, so its eigenvalues are not spectral invariants (they flip $0.04/0.15/242$ across normalizations). Cannot settle the semilocal question.
- **[`e1g_ccm_faithful_prolate.py`](e1g_ccm_faithful_prolate.py)** — the genuine band-in-$s$ concentration operator (true projections, invariant spectrum, the `e1f` bug fixed). **Reweighting-blind**: any spectral multiplier (a random non-arithmetic one included) gives the identical spectrum by diagonal similarity, so it is L-function-blind by construction.
- **[`e1h_ccm_degree_prolate.py`](e1h_ccm_degree_prolate.py)** — the genuine degree-domain operator $W_{\lambda,S}=(H+\tfrac12)^2+\lambda^2 N_S$. **Reads moments, not arithmetic** (a single prime's factor is just a frequency $\log p$; the joint $\{2,3,5\}$ test lands in a matched non-arithmetic cloud's body). Validated identity: the archimedean Hardy-Titchmarsh scaling operator **is** the Meixner-Pollaczek Jacobi matrix ($\alpha_k{=}0$, $\beta_k^2{=}k(k{-}\tfrac12)$, exact). The adversary steelmanned the positive case and killed its own $z{=}{+}2.29$ false positive.

The full session synthesis is [`../../docs/03_research/eight_angle_sweep_2026-06.md`](../../docs/03_research/eight_angle_sweep_2026-06.md) (LEARNINGS #111-#117).

## The metaplectic operator, finite-local model (e1i, 2026-06-25)

The un-eliminated route the three OP-data surrogates left open is **CCM's deferred metaplectic operator**. Since e1f/g/h are blind because they build from the modulus-squared measure $dm_S$, and the metaplectic arithmetic (the Weil index, the quadratic character) is a phase/sign, [`e1i_metaplectic_weil_index.py`](e1i_metaplectic_weil_index.py) builds the **finite Weil representation over $\mathbb{F}_p$** as a computable model of that sign-structure and tests whether it survives the positivity compression. Builder→adversary, five corrections applied (LEARNINGS #118; `../../scratchpad/ccm_metaplectic/03_adversary.md`).

- **Exact gate (the metaplectic analogue of e1h's Meixner-Pollaczek).** (G2) the trace of the normalized metaplectic Fourier **is the Weil index**, $\mathrm{Tr}(F_p)=\varepsilon_p$ ($=1$ if $p\equiv1$, $i$ if $p\equiv3 \bmod 4$, exact, Gauss's sign theorem); (G5) the double-cover **2-cocycle** $F^2$ vs $\rho(m(-1))=(-1|p)\cdot\text{parity}$ is non-trivial exactly for $p\equiv3\bmod4$ (gap $0/2$). A citable validated identity; the torus character-multiplicativity alone would pass with the trivial sign, so the cocycle is the real metaplectic-sign test.
- **Finding: finite-local signature-blindness.** The scalar Weil index **cancels** in $g^*g$ (T1, $|\varepsilon_p|^2{=}1$), and a **concentration-matched, shear-free Arbiter** (a cloud of smooth quadratic chirps) places the genuine chirp's Sonin-concentration spectrum in the body for all five tested primes (medians $\sim$18–23%). False-negative-probed: a planted $\theta{=}4$ outlier reads 100% tail, the genuine reads 2–16% deep body. So the finite-local quadratic sign-structure does **not** survive the finite Sonin positivity compression as arithmetic discrimination.
- **Scope (does NOT close the door).** This is the finite-local quadratic (Gauss-sum) sign-structure at one place, **not** CCM's actual semilocal $W_{\lambda,S}$ (the global $\widetilde{SL(2,\mathbb{A}_S)}$ operator from the degree-1 Euler factors). e1i does **not** eliminate the metaplectic route; it sharpens the map — the metaplectic arithmetic is genuine, but the place it survives positivity (if anywhere) is the global $S\to\infty$ assembly, which **is** M4.
