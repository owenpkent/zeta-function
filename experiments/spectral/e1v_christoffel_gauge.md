...
**Date**: 2026-08-08. **Role**: BUILDER (this probe), executing the LEARNINGS #171 handed-forward item 2 / [`PHASE_STATE.md`](../../PHASE_STATE.md) "Next steps" item 2 (the Christoffel-corpus rung) as a **local build** rather than a literature sweep, and discharging [`e1u`](e1u_canonical_chain.md) VERIFIER target 6. **Status**: built and adversarially probed in one session; 26/26 self-tests full, extended and quick (was 23/23 before the V8 drift section of 2026-08-09); ADVERSARY round **self-run** ([`_e1v_adversary.md`](_e1v_adversary.md), 6 posed attacks run + 1 unposed; five did not land, one closed an open item, one independently confirmed V7, and **case 3 LANDED and forced a correction to the action recommendation**), with case 7 **not run and open**. No second party has read this rung.

**Artifacts**: [`e1v_christoffel_gauge.py`](e1v_christoffel_gauge.py) (23/23 full, ~5 s warm; 23/23 quick, ~0.3 s, saves nothing and does not clobber the npz, md5-verified), `e1v_christoffel_gauge.npz` (full mode only; byte-reproducible across consecutive runs, md5-verified), `e1v_christoffel_gauge_extended.npz` (written only by `--extended`, which adds whichever of the lambda = 4.0 / 4.5 configs are cached and leaves the tracked default npz byte-stable), shared build cache `_cache/e1t_build_*.npz` (gitignored; a cold machine pays ~5.5 min for the 11-build grid and a further ~40 min for the four extended builds, measured: 439 s / 502 s / 549 s at $N = 66$, 925 s at $N = 91$).

**Verdict in one line**: `christoffel_gauge_carries_arithmetic = NO, POINTWISE`. The germ-length blowup that e1u measured is now a **theorem with an explicit potential-theoretic rate** that depends only on $(g, T, M)$, the tightness residual is **family-blind once the gap is equalized** (means 1.21 / 1.31 / 1.53, spread 1.27x), and the whole functional is **provably unable to see $\mathbb{Q}$-linear independence** because it is continuous in the atom positions while lattices are dense (demonstrated: snapping the true zero set onto $(1/D)\mathbb{Z}$ moves the per-atom rate by $\le 2.4\cdot 10^{-8}$ at $D = 10^6$, uniformly in $M$). **[CORRECTED after adversary case 3.]** The corpus question as posed asked for $\lambda$-**uniform** control, which is a statement about the SEQUENCE, so the obstruction does **not** close it: continuity at every finite index is fully compatible with an arithmetic-sensitive limit (probe G witnesses this with normalized quadratic Weyl sums, each a trigonometric polynomial in $\alpha$ and hence rational-blind by exactly this argument, whose limits are $1/\sqrt q$ at $\alpha = p/q$ and $0$ at irrational $\alpha$, reproduced to 4-5 digits). What the obstruction DOES close is the pointwise **growth-bound** route. An independent scoping gap points the same way (the corpus studies $\lambda_n$ for a FIXED measure as $n \to \infty$, whereas this object is the diagonal $n = M(\lambda)-1$ with the measure moving). Net recommendation: **re-scope the sweep to its sequence-level half, where Killip-Simon sum rules already live; do not skip it.**
...
| Q2 | The proved bound holds and is order-tight: $\rho = O(1)$, not $O(M)$ | $\rho$ growing with $M$ would mean the geometric law is not the leading order and would REOPEN the pointwise route | **CONFIRMED on the buildable window** ($\rho \in [1.15, 3.32]$, corr$(\rho, M) = +0.55$), **but SCOPED by V8**: extrapolating the geometry predictor, $\rho \sim 0.48\log T$ without saturating, so the bound is not asymptotically order-tight. The exit's REOPEN branch nevertheless does **not** fire, because the growth is reproduced in full by an arithmetic-free surrogate (V8c) |
...
### V8 The lambda-drift: geometry, and it does not saturate (added 2026-08-09)

The one measurement #172 handed forward. The gap-equalized residual $\rho_{\text{eq}}$ drifts upward with $\lambda$ for every family, and the adversary's fixed-gap-fraction probe had already shown $\rho$ saturates in $M$ at fixed $g/T$, pointing at the gap FRACTION (shrinking as $T = 2\pi\lambda^2$ grows at fixed $g$) rather than at $M$. V8 tests that directly by replacing each real zero configuration with the EQUALLY SPACED one of the same $(g, T, M)$ after band equalization. Four extra builds were made for this ($\lambda = 4.0$ for all three families, $\lambda = 4.5$ for zeta; about 40 min of compute).

| build | M | $g$ | $T$ | $\rho_{\text{eq}}$ | $\rho_{\text{geom}}$ | residual |
|---|---|---|---|---|---|---|
| BEUR 2.2 / 2.6 / 3.0 / 4.0 | 8 / 16 / 30 / 76 | 16.42 / 16.24 / 16.01 / 15.61 | 29.2 / 41.0 / 56.9 / 100.0 | 1.154 / 1.223 / 1.242 / 1.583 | 1.077 / 1.196 / 1.321 / 1.546 | +0.078 / +0.026 / -0.080 / +0.037 |
| D-H 2.2 / 2.6 / 3.0 / sqrt13 / 4.0 | 8 / 18 / 32 / 58 / 80 | 17.19 / 14.89 / 14.44 / 14.41 / 14.40 | 28.4 / 40.4 / 55.3 / 81.8 / 100.4 | 1.083 / 1.264 / 1.350 / 1.538 / 1.670 | 1.067 / 1.221 / 1.344 / 1.493 / 1.575 | +0.016 / +0.043 / +0.006 / +0.045 / +0.094 |
| ZETA 2.2 / 2.6 | 8 / 16 | 14.135 | 30.4 / 40.9 | 1.151 / 1.173 | 1.110 / 1.222 | +0.041 / -0.049 |
| ZETA 3.0 / sqrt13 / 4.0 (all V7-flagged) | 26 / 48 / 72 | 14.135 | 56.5 / 79.3 / 98.8 | 2.204 / 1.607 / 1.556 | 1.337 / 1.475 / 1.569 | +0.867 / +0.132 / -0.013 |

**V8a: the drift is GEOMETRY.** Over the 11 builds the declared V7 rule does not flag, the $(g, T, M)$-matched equal-spacing surrogate predicts the real tightness residual with correlation $+0.967$ and worst absolute residual $0.094$ (default grid without the extended points: $+0.936$, $0.080$ over 9). A surrogate with no arithmetic content at all, matched only on gap, support and count, reproduces the entire $\lambda$-drift to within a tenth.

**V8b: it does NOT saturate.** With the surrogate licensed, the predictor costs nothing to evaluate far beyond the buildable window ($g = 13.6$ fixed by the FE budget and NOT growing with $\lambda$; $T = 2\pi\lambda^2$; $M = 2\times$ the Riemann-von Mangoldt count in $[g, T]$, used here as a counting model for a synthetic geometry study and not as an input to any discriminating claim):

| $\lambda$ | 2.2 | 3.0 | 4.0 | 5.0 | 6.0 | 8.0 | 11.0 | 15.0 |
|---|---|---|---|---|---|---|---|---|
| $T$ | 30.4 | 56.5 | 100.5 | 157.1 | 226.2 | 402.1 | 760.3 | 1413.7 |
| $M$ | 6 | 22 | 58 | 112 | 186 | 406 | 920 | 1988 |
| $\rho_{\text{geom}}$ | 1.143 | 1.332 | 1.570 | 1.762 | 1.924 | 2.190 | 2.494 | 2.797 |

The $\lambda$ steps are uneven, so raw increments are not comparable; in the right coordinate the slopes $d\rho/d\log T$ are $0.303, 0.414, 0.430, 0.444, 0.462, 0.477, 0.489$, converging to $c \approx 0.48$. So $\rho \sim 0.48 \log T$: **the drift grows logarithmically without bound.** Since $\rho$ is the ratio of the measured quantity to the proved bound, this says the Chebyshev bound of Theorem V2, which is order-tight on the buildable window ($\rho \in [1.08, 1.67]$ unflagged), **loses a slowly growing factor asymptotically and is not asymptotically sharp.** V2b is scoped accordingly: "the geometric rate is the leading order" is a statement about the buildable window, not a limit statement.

**V8c: the growing residual is geometry, not room for arithmetic.** This is what stops the pre-registered Q2 exit from firing. The exit said a growing $\rho$ "would mean the geometric law is NOT the leading order and the residual is where everything lives", which would REOPEN the pointwise route. The first half happened; the second does not follow, because the growth is exhibited **in full by the equally spaced surrogate**, a configuration with zero arithmetic content, and V8a shows the real configurations track that surrogate to $0.094$. The bound leaves a growing gap, but the gap is filled by geometry, not left open for arithmetic.

**A note on the V7 flag.** Three of the five zeta builds are now flagged near-degenerate (3.0, sqrt13, 4.0). The flag is conservative rather than diagnostic of distortion: ZETA 4.0 carries a tight pair yet its residual is $-0.013$, the smallest in the table. The rule earns its keep on ZETA 3.0 and costs little elsewhere.
...
5. **Theorem V2 is one-sided, and now measurably not sharp.** [**UPDATE
   2026-08-17: the sharp form is SUPPLIED at the level of the exponential
   rate by [`e1z_v2_sharp.md`](e1z_v2_sharp.md). The correct exponent is not
   $G$ but the equilibrium discrepancy
   $\Gamma(\sigma) = \max_E U^\sigma - U^\sigma(0)$ of the atoms' limiting
   distribution; Theorem V2 is exactly the $\sigma = $ equilibrium case, where
   $\rho \to 1$; and the missing factor is
   $\Gamma_{\rm unif}/G = \frac12\log T + O(1)$ for equal spacing, so the
   $0.48$ below is the finite-$T$ approach to exactly one half. V8b's
   predictor is independently replicated there 8/8. The typing is unchanged:
   $\Gamma$ is a functional of $\sigma$ alone.**] It lower-bounds $1/\lambda_M(0)$; no matching upper bound is proved. V8 quantifies the gap: on the buildable window the bound is order-tight ($\rho \in [1.08, 1.67]$ unflagged), but the geometry predictor gives $\rho \sim 0.48\log T$, so asymptotically the bound loses a logarithmic factor. That factor is geometry (V8c), not room for arithmetic, but a sharp version of the theorem would have to capture it. Probe D strengthens it: at **fixed** gap fraction $g/T = 0.25$, $\rho$ stays in $[1.30, 1.42]$ while $M$ grows 12-fold ($8 \to 96$), visibly saturating, so the $O(1)$ reading is structural rather than grid-local. Probe D also identifies what the observed $\lambda$-drift of $\rho_{\text{eq}}$ actually is: not an $M$ effect but the gap fraction $g/T$ shrinking as the window grows at fixed $g$.
6. **The ADVERSARY round was self-run.** [`_e1v_adversary.md`](_e1v_adversary.md): six posed attacks run (cases 1, 2, 3, 4, 5, 6), five failed to land, **case 3 LANDED** (the obstruction is strictly pointwise, so the action drawn from it was too strong; corrected throughout), one (B) closed an open item, and one unposed attack (F, verifying the atom sets against e1u's tracked npz) **independently confirmed V7**: all 22 atom counts reproduce exactly and every germ length reproduces to $\le 1.6\cdot10^{-8}$ except ZETA 3.0 Face A at $3.7\cdot10^{-4}$, i.e. the one build the degeneracy rule flags is the one that fails to reproduce across machines, for the stated reason. Case 7 (independent K1/determinism re-verification) was **not** run and remains the open attack surface, together with the standing fact that no second party has read the rung.
...
1. **The corpus sweep should be RE-SCOPED, not skipped. [CORRECTED after adversary case 3; the first version of this item overreached.]** PHASE_STATE next-step 2 asks for "ANY mechanism giving $\lambda$-uniform Christoffel control at a point against measures with atoms at $\mathbb{Q}$-linearly independent positions, for an Euler + lattice reason". The pointwise version of that request is unsatisfiable in principle (continuity versus a dense complement). **But the request as written is $\lambda$-UNIFORM, i.e. a statement about the sequence, and the obstruction does not reach it**: probe G exhibits a family of functionals each continuous (hence arithmetic-blind by exactly this argument) whose limit is arithmetic-sensitive. So the correct action is to split the sweep, not to cancel it: the pointwise **growth-bound** sub-corpus is dead (and the four-level caution is genuinely sharpened there, since Lubinsky-type universality was already out as bulk-asymptotic), while the **sequence-level** sub-corpus is exactly where the surviving question lives, i.e. Killip-Simon sum rules, the register the #170 survey already named as the proven neighbouring template. A second, independent reason to re-scope is untouched by any of this: the degree-regime mismatch in honest limit 2.
2. ~~**The $\lambda$-drift of $\rho_{\text{eq}}$**~~ **ANSWERED 2026-08-09 by V8.** The drift is geometry (an arithmetic-free $(g,T,M)$-matched surrogate predicts it with correlation $+0.967$, worst residual $0.094$, over 11 builds including four new ones at $\lambda = 4.0, 4.5$), and it does **not** saturate: $\rho \sim 0.48\log T$. Consequences recorded above: V2b is scoped to the buildable window, Theorem V2 is not asymptotically sharp, and the Q2 exit does not fire because the growth is geometry-typed. What remains open in this coordinate is only the SHARP form of the theorem: an upper bound capturing the $\log T$ factor. **ANSWERED 2026-08-17 at the level of the rate by [`e1z_v2_sharp.md`](e1z_v2_sharp.md) (the exponent is $\Gamma(\sigma)$, the factor is exactly $\frac12\log T$); what is still open is a two-sided FINITE-$M$ inequality with effective error terms, and the unequal-weight case.**
...
3. **Attack the continuity obstruction as stated.** Does it also kill every *sequence-level* statement a sum rule could make? It should not. [OUTCOME: **THE ATTACK LANDS, partially** (probe G, 2026-08-09). The obstruction itself is valid and untouched: no finite-$M$ Christoffel functional can detect $\mathbb{Q}$-linear independence. But the ACTION drawn from it was too strong. The sweep's target was $\lambda$-**uniform** control, a sequence-level statement, and continuity at every finite index is compatible with an arithmetic-sensitive limit: normalized quadratic Weyl sums $c_N(\alpha) = |N^{-1}\sum_{n\le N} e^{2\pi i n^2\alpha}|$ are trigonometric polynomials in $\alpha$, hence rational-blind by exactly this argument, yet $c_N \to 1/\sqrt q$ at $\alpha = p/q$ and $\to 0$ at irrational $\alpha$ (reproduced to 4-5 digits at $N = 10^5$). Corrected in the verdict line, Handed forward 1, LEARNINGS #172, PHASE_STATE and TODO: **re-scope the sweep, do not cancel it.** The independent reason to re-scope (honest limit 2, the degree regime) is unaffected.]
