...
No candidate has a ✅ on (xi-xiii) — the Hodge index positivity. **This is the universally open construction problem.** After R3.5 (no-shortcut theorem) and R3.6 (deep dive into Connes-Consani C3 formulations), all NCG-style candidates fail K1 on this slot; only the geometric route (intersection theory + Hodge index on a constructed surface) remains as a structural escape.
...
**The (xi)-(xiii) column** was sharpened by R3 → R3.5 → R3.6 sequence. R3 found C1, C2 fail K1 (RH-equivalent) and C3 K1-uncertain. R3.5 proved a no-shortcut theorem: every trace-formula NCG framework has positivity ⟺ RH, so NCG-only approaches universally fail K1 by structure. R3.6 did a deep dive on Connes-Consani's C3 formulations (arithmetic site, hyperrings, characteristic-one geometry) and found they all fit R3.5's hypothesis (despite their sophistication), so they probably also fail K1. Net effect: Connes and Connes-Consani (xi)-(xiii) move from 🟡 back to ❌, reflecting the strengthened evidence that no NCG-only formulation escapes K1.
...
- ~~**R3**: For Connes-Consani, identify whether the noncommutative positivity conjecture is K1 (RH-equivalent) or whether it has an independent constructive proof candidate.~~ **Complete** ([2A_R3_connes_positivity.md](2A_R3_connes_positivity.md)). Companion deep-dive: [2A_connes_dossier.md](2A_connes_dossier.md) covers the framework's origin (1995, 1999), adèle class space construction, trace formula structure, the $\mathbb{F}_1$ reframing (Connes-Consani 2009-), strengths / limitations / open questions, and the post-R-series scorecard (3 ✅ / 6 🟡 / 4 ⏳ / 3 ❌ with the K1 wall at (xi)-(xiii)). The Connes-Consani framework hosts at least three positivity conjectures with different K1 status: **C1 (Weil-Bombieri positivity in the adèle class space) fails K1 — provably RH-equivalent.** **C2 (Hamiltonian self-adjointness, the Hilbert-Pólya formulation) also fails K1.** **C3 (arithmetic-site / hyperring positivity, post-2008 refinements) is K1-uncertain** — not provably equivalent to RH, but no independent proof exists either. Scorecard update: Connes-Consani (xi)-(xiii) move from ❌ to 🟡 (the framework asks the right questions but hasn't answered them). **Cross-architecture connection**: R3 aligns with Arch 3's 3F-3I finding — both confirm Weil positivity is hard to derive without RH-strength input, whether from the analytic side (prime sum cancellation tightness) or the noncommutative-geometric side (Hilbert space positivity). The geometric route (Arch 2's intersection-theory Hodge index) remains the only direction where positivity might emerge for free.
- ~~**R3.5**: Does ANY noncommutative-geometric framework escape K1?~~ **Proved** ([2A_R3_5_K1_universality.md](2A_R3_5_K1_universality.md)). **No-shortcut theorem**: every trace-formula NCG framework $(H, F, \mu, T)$ with standard spectral identification (spectrum of F = imaginary parts of zeta zeros) has positivity P ⟺ RH. Three positivity types (P-SA self-adjointness, P-Q quadratic form PSD, P-OP operator non-negativity) all fall under the theorem. Proof: the trace identity + spectral identification reverses (P → RH gives the framework's hypothesis; RH → P follows because real spectrum → self-adjoint operator with PSD square). **Consequence**: NCG-only approaches universally fail K1 by structure; the K1 wall is universal across trace-formula NCG. **The unique escape route is geometric**: intersection theory + Hodge index theorem on a constructed surface provides positivity from the SIGNATURE of the intersection form, structurally different from trace-formula positivity. Sharpens R3's verdict on C3 from 🟡 toward ❌. **Sharpens the central understanding**: constraints (xi)-(xiii) are not just "open" but specifically THE escape route through which any RH proof via Architecture 2 must go. The theorem is essentially folklore in NCG circles; the contribution is making it explicit in the 17-constraint framework's language.
- ~~**R5**: Investigate prismatic cohomology for Spec(W(ℤ)).~~ **Analyzed** ([2A_R5_prismatic_cohomology.md](2A_R5_prismatic_cohomology.md)). δ-rings (used in BMS prismatic cohomology) are essentially "Λ-rings at one prime p"; Borger's Λ-structure naturally projects to δ-rings at each prime, making prismatic cohomology applicable to Spec(W(ℤ)). **Predicted impact**: closes constraints (iv)-(vii) for Borger if it applies as expected. **K1 status**: still fails — prismatic cohomology has its own trace formula structure that falls under R3.5. So R5 is infrastructure progress, not K1 escape. Five open verification questions (R5.1-R5.5).
- **Path forward** ([2A_path_forward.md](2A_path_forward.md)): synthesis of R1-R5 into a strategic plan. Develop BOTH hybrid candidates (Λ-blueprints and Borger + Connes) in parallel, use prismatic cohomology for both, attack Hodge index on whichever track reaches it first. 5-10 year multi-paper research program, success probability < 50%.

- ~~**R3.6**: Deep dive into Connes-Consani C3 (arithmetic site, hyperrings, characteristic-one geometry).~~ **Complete** ([2A_R3_6_arithmetic_site.md](2A_R3_6_arithmetic_site.md)). A focused analysis of whether the most aggressive Connes-Consani refinements escape R3.5's no-shortcut theorem. **Verdict**: probably not. The arithmetic site (a topos with hyperring-valued structure sheaf and Frobenius-like endomorphism Frob_ar) hosts at least four positivity formulations (C3a-C3d). All four either fit R3.5's hypothesis (P-SA / P-Q / P-OP type) and hence fail K1, or are not strong enough to imply RH on their own. **Connes / Connes-Consani (xi)-(xiii) sharpened from 🟡 to ❌** in the summary scorecard. Some uncertainty remains in the "purely sheaf-theoretic positivity" case (C3d), but it would have to BYPASS the trace identity entirely to escape R3.5 — and no such formulation has been articulated in the published literature. **Silver lining**: the Connes-Consani categorical machinery (topos theory, hyperrings, characteristic-one geometry) might serve as INFRASTRUCTURE for the geometric route in Architecture 2 — even if it doesn't directly provide positivity, it could provide the right notion of "scheme below ℤ" or sheaf-theoretic tools for the surface. R3.6.3 surfaces this as a follow-up research direction.
- ~~**R4**: Explore the Borger Frobenius + Connes trace formula hybrid.~~ **Analyzed** ([2A_R4_borger_connes_hybrid.md](2A_R4_borger_connes_hybrid.md)). The natural hybrid combines Borger's discrete Adams operations ψ_p with Connes' continuous ℝ*_+-action on the adèle class space via the bridge U_t = ∏_p U_{log p}^{t/log p}. Predicted scorecard: **~8 ✅ / 5 🟡 / 3 ❌** — similar to Λ-blueprints (R2.5) but via different mechanism. **K1 status**: fails K1 (per R3.5), since the trace identity is inherited from Connes. **Value**: provides infrastructure for the geometric route — a candidate Hilbert space H = L²(W(ℤ), μ) where Borger's surface side and Connes' trace formula side coexist. Five technical research questions surfaced (R4.1-R4.5) for someone making the hybrid rigorous. **The hybrid and Λ-blueprints are complementary**: Λ-blueprints prioritize clean surface structure; Borger + Connes prioritizes trace formula. Both inherit K1 failure on (xi)-(xiii); both could provide infrastructure for the geometric escape route.
