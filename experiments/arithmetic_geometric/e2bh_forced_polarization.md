...
## Prior art (filing condition, 2026-08-26)

The negative-eigenvalue datum is banked prior art, not a finding of this probe: Arch 3C-3D's Gram detector [`e3c2_weil_gram`](../positivity/e3c2_weil_gram.py) already carries $M^{\mathrm{DH}}$ negative eigenvalues with a stability witness; LEARNINGS #199 certifies the same window from the control side; and the unit-index-weight reading is #202(iii)'s Sylvester-inertia datum (off-line pairs at unit index weight), which this probe re-measures in a 21-dim window family. Native here is only the surgery framing: the deleted-mode line profile as a K1-localization meter.
