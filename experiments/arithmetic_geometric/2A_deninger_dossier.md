...
>
> **Caveats upfront**: Deninger's program is famously the *most conjectural* of the major Arch 2 candidates. I have partial familiarity with the Deninger 1991-2010 papers and the Leichtnam-Deninger collaborations; the post-2010 work is less covered.
>
> **UPDATE 2026-06-02 (correction to the body below):** the claim throughout this dossier that "the foliated space $X$ has never been constructed" is now OUTDATED. Deninger constructed the foliated dynamical system via sheafified rational Witt vectors $W_{rat}(X)$ for normal schemes of finite type over $\mathrm{Spec}(\mathbb{Z})$, with an $\mathbb{R}$-flow whose periodic orbits biject with closed points (*Dynamical systems for arithmetic schemes*, arXiv:1807.06400, 2018; motivated by Kucharczyk-Scholze; verified against the original). The regularized-determinant Lefschetz formula is moreover proven for genuine 3-dimensional Riemannian foliated systems (Deninger 2024). So $X$ and the flow now EXIST; what remains open is the leafwise cohomology $H^*_F(X)$ with its Poincare duality and, above all, its **polarization** (the Hodge-index signature whose positivity is RH). The "central object doesn't exist" blocker below should be read as "the cohomology, its duality, and its polarization remain open." See [`../../docs/03_research/spec_z_cohomology_landscape.md`](../../docs/03_research/spec_z_cohomology_landscape.md) §7.
