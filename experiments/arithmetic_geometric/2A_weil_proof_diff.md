...
> **Scope note (2026-07-01):** "deepest" is template-relative, a property of Weil's three-slot machine, not of the transfer problem as a whole. The Stepanov-Bombieri counting engine is a second machine with two slots (Frobenius, Riemann-Roch) and no positivity slot at all; over $\mathrm{Spec}(\mathbb{Z})$ it walls at the Frobenius slot (R1) plus the missing diagonal, never meeting the Hodge index. See [`../../docs/03_research/stepanov_engine_audit.md`](../../docs/03_research/stepanov_engine_audit.md) (LEARNINGS #145).
