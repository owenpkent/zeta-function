...
| `engine.py`, `test_engine.py` | The Reduction Engine loop driver (increment 4): runs one cycle (ingest -> falsify -> route -> score -> report) over a candidate, prints the anti-theater kill tally, writes a monotone log to `_cache/`, and carries the construction-registry cross-check of `dh_buildable`. |
| `lean_hook.py` | The Lean closability hook (increment 5): the soundness floor. Advisory textual sorry audit by default; authoritative `lake build` + sorry-warning scan with `run_build=True`. Promotes nothing. |
| `fq_shadow.py` | Generative Engine 6d: the function-field shadow, the positive mirror of the D-H discipline. Genuine $\mathbb{F}_q$ curves (Frobenius on the $\sqrt q$ circle, RH = Hasse) pass; an off-line forgery is killed. See [`docs/03_research/generative_engine.md`](../../docs/03_research/generative_engine.md). |
| `generator.py`, `test_generator.py` | Generative Engine 6a + 6c: the move-library generator and the quality-diversity archive. Proposes structure-preserving variants from a Weil/$\mathbb{F}_q$ seed and prunes each with the evaluate stack (reusing the oracle's static disqualifiers + the $\mathbb{F}_q$ shadow), binning by failure cell. The first organ that proposes. |
| `branch_specs.py`, `test_branch_specs.py` | Generative Engine 6e: new-branch spec generation. Reads the forcing question off the top asserted-vs-proven gap, writes the M4 required-property spec (one blind-spot property), and computes each candidate's residual, generating the all-roads convergence (9 converge on the open polarization, 8 bracket out) instead of asserting it. |
| `transfer_search.py`, `test_transfer_search.py` | Generative Engine 6b: the bridge-finder. Feature-match retrieval over a corpus of proven positivity theorems against the M4 residual signature (indefinite vs definite as the hard gate). Rediscovers the Bost-Connes pinning import, demotes Lee-Yang, surfaces Hodge-Riemann / Alexandrov-Fenchel; retrieval is a prior, not a verdict. |
