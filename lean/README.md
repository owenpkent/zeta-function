...
| #FF-1      | FunctionFieldRH.lean          | Lever B, the eigenvalue-extraction link (genus 1): a non-real root of `X²−tX+q` has `|α|² = q` (`eigenvalue_modulus`, via Vieta on the conjugate); `t² < 4q ⇒ the roots are non-real` (`root_nonreal`); hence `NegDef ⇒ |α| = √q` (`functionfield_RH_elliptic_of_hodge`, wiring #2G-1). PROVED (no sorry). |
| #FF-geom   | FunctionFieldRH.lean          | Lever B gap: the geometric Castelnuovo-Severi step `hodge_index_curve_elliptic` (the primitive intersection form on `C × C` is negative definite). Sorry (Mathlib has no algebraic-curve intersection theory). The one open link in the function-field RH chain. |
