...
| #EF-arch   | ExplicitFormula.lean          | The archimedean kernel. Kernel part **DISCHARGED**: `digamma := logDeriv Complex.Gamma`, `digamma_eq` proved, `archKernel` concrete, and the basic algebraic identities `digamma_add_one` (`ψ(s+1)=ψ(s)+1/s`) and `digamma_reflection` (`ψ(1-s)-ψ(s)=π cot(πs)`) **PROVED** (no sorry) from `Complex.Gamma_add_one` and `Complex.Gamma_mul_Gamma_one_sub`. Remaining: the integral pairing (#EF-class). |
