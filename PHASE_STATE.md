...
0. **Two concrete leads from the reference-library read-through (session 006, 26 deep notes in [`docs/03_research/reading_notes/`](docs/03_research/reading_notes/)).** The literature confirms the 2Q/2R reduction and locates the gap identically (the signature is the open step). The two most concrete objects to push: (a) **Bhatt-Lurie's Cartier-Witt stack `WCart`** ("de Rham relative to F_1"; a global Frobenius that contracts `WCart^HT` to the de Rham point; a Sen operator `Theta = u^Theta` = the literal cyclotomic-flow generator) -- the finite-places substrate for Direction 4.6; (b) **Connes-Consani's square `N^2-hat`** (Newton polygons in `ZxZ` with Frobenius correspondences `Psi(lambda)`, composition law) -- the product object already built, missing only a SIGNED intersection pairing (idempotent char-1 ops). Putting a signature on the Connes-Consani square, or extracting one from the WCart Sen operator, is Direction 8 in concrete form.
...
- Git commit: `8873c1b` (session 006; PUSHED). Session 006 = the reference library: 26 owner-supplied PDFs organized into a gitignored `references/` tree + tracked bibliography index, and FULL-DEPTH reading notes for all 26 (section-by-section technical content, ~6,200 lines, in `docs/03_research/reading_notes/`). Two concrete leads surfaced (Bhatt-Lurie WCart Sen operator; Connes-Consani square). Prior: `0de058e` (session 005: 2Q #25, 2R #26, the Lean Weil-explicit-formula thread; build green, sorry count 23).
