...
3. **Lean.** Complete the 2H Faltings-Hriljac statement's dependencies if/when Mathlib gains canonical heights; otherwise the Weil explicit formula (LEARNINGS #17) remains the highest-leverage Mathlib target.
...
- Git commit: `8cd4096` (session 004; pushed). Local-height decomposition validated (2P).
- LEARNINGS.md last finding: #24 (complete single-arithmetic-surface Hodge-index picture: ranks 1-4 PosDef + full Silverman local decomposition = h_hat). Session 004 added #20 (input-side place-type / Euler fingerprint), #21 (2G function-field signature = Hasse-Weil), #22 (2H Faltings-Hriljac over Spec(Z)), #23 (2I archimedean local height), #24.
