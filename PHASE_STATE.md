...
3. **Lean.** The Weil explicit formula + positivity criterion now have a green typed SCAFFOLD (`ExplicitFormula.lean`, #EF-1/#EF-2, session 005). Next Lean rungs, in order of tractability: (a) #EF-arch -- contribute a `digamma` / Γ-factor-log-derivative to Mathlib (genuinely missing, upstreamable on its own); (b) discharge `zeta_ne_zero_re_gt_one` (#MB-2) and `zeta_functional_equation` (#MB-6) from existing Mathlib (`completedRiemannZeta`, `riemannZeta_one_sub`) -- these are PRESENT and should be provable now; (c) the 2H Faltings-Hriljac dependencies if/when Mathlib gains canonical heights.
...
- Git commit: `1b4701d` (session 005; 2Q bidegree obstruction committed, not pushed). Uncommitted: Lean `ExplicitFormula.lean` scaffold (Weil explicit formula + positivity criterion, #EF-1/#EF-2; build green) + README/PHASE_STATE updates.
