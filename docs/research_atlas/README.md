...
|---|---|---|
| **3: Direct positivity** (Weil / Li) | 3A, 3B, 3B.2, 3C, 3D, 3F, 3G, 3H, 3I complete | Small-n Li-positivity does NOT distinguish zeta from Davenport-Heilbronn (3B), but large-n DOES: 3B.2 witnesses $\lambda_n^{DH} < 0$ at $n = 4 \times 10^5$. Gram-matrix wrong-approach detector: $M^{DH}$ has negative eigenvalues at $K = 30$, $T_{\max} = 200$ (3C, 3D); Selberg-class cross-cut on $\chi_3, \chi_4$ confirms direction-selectivity (3C.3, 3D.2). **Weil-form duality (3F, 3G, 3H, 3I): the cancellation tightness in the prime side is specifically a feature of $\zeta$'s pole at $s=1$, not of Euler products generally. For $\chi_3$ (Euler product, no pole), cancellation is 100× looser. Tested whether $\chi_3$'s milder cancellation gives unconditional access via Siegel-Walfisz: BLOCKED — partial summation loses the cancellation and the unconditional bound is too loose by factor 30-120. Same circularity wall as $\zeta$. The Weil-form route is structurally blocked across the Selberg class.** |
...
| **4: Analytic** (zero-free regions) | 4B, 4D, 4D.2, 4E, 4E.2 complete | 1D LP saturates Fejér $\cos(\pi/(n+2))$ (4B). The d-variate generalization with single-coefficient objectives decomposes (4D, 4D.2): $\max c_{1, \ldots, 1} = (2\cos(\pi/(N+2)))^d$ exactly equals the tensor product of 1D Fejér optima. **Sum-of-coefficient objectives do NOT decompose (4E, 4E.2):** $\max c_{1,1} + \alpha c_{2,2}$ at bidegree $(2,2)$ exceeds the Cauchy-Schwarz tensor bound for all $\alpha > 0$, with peak relative gap **+25.00% at $\alpha = 3$** (LP = 4, tensor = 16/5). LP-optimal $P$ has clean rational coefficients ($c_{1,1} = 8/5, c_{2,2} = c_{0,2} = c_{2,0} = 4/5$) and a structural non-factorization in $(\theta+\phi, \theta-\phi)$ coordinates. Extending to 3-term sums at $N = 3$ multiplies the gap by 8.66x. **First genuinely 2D auxiliary inequality** in the experimental thread; translation to a Mossinghoff-Trudgian zero-free constant requires Heath-Brown / Pintz two-height explicit-formula bookkeeping (queued as 4E.3). |
...
- **Multivariate trig-polynomial inequalities can exceed tensor products of 1D Fejér (4E / 4E.2):** the LP for $\max c_{1,1} + 3 c_{2,2}$ at bidegree $(2,2)$ beats the Cauchy-Schwarz tensor bound by +25%. Whether this translates into a zero-free region constant improvement requires Heath-Brown / Pintz two-height explicit-formula bookkeeping (open).

**Why it failed (the classical approach):**
The 1D tools are exhausted. The trigonometric inequalities that drive the classical method (variants of $3 + 4\cos\theta + \cos 2\theta \geq 0$) cannot be improved beyond Fejér. Vinogradov's exponential sum method seems to have a structural ceiling at exponent $2/3$. Reaching $\sigma = 1/2$ from $\sigma \approx 1$ requires not just better estimates but entirely different techniques. The 4E/4E.2 multivariate finding suggests there may still be room in the 2D trig-polynomial regime not previously explored.
