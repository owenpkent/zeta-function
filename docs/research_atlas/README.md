...
| **2: Arithmetic-geometric** (Deninger / $\mathbb{F}_1$) | 2A, 2B complete | Weil RH verified for $E: y^2 = x^3 + x + 1$ over $\mathbb{F}_5$: Frobenius eigenvalues at $\alpha = -1.5 \pm \sqrt{11}/2 \, i$ with $\|\alpha\|^2 = 5$ exactly; point counts match Weil formula at $k=1, ..., 6$. **2A: Weil-proof diff table** (Lefschetz + Poincaré + Hodge index vs missing analogues over $\mathrm{Spec}(\mathbb{Z})$) traces precisely what each program (Connes, Deninger, $\mathbb{F}_1$) addresses and what's missing. **The obstruction is constructive, not analytic**: the proof template is well-understood; the underlying object on which to instantiate it has not been built, and each of the three programs addresses only one corner of the (Frobenius / surface / positivity) obstruction triangle. |
