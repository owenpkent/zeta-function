...
|---|---|---|
| **3: Direct positivity** (Weil / Li) | 3A, 3B, 3B.2, 3C, 3D complete | Small-n Li-positivity does NOT distinguish zeta from Davenport-Heilbronn (3B), but large-n DOES: 3B.2 witnesses $\lambda_n^{DH} < 0$ at $n = 4 \times 10^5$ via on-line asymptotic + off-line correction. The Weil quadratic form via Gram-matrix eigenvalue analysis IS a wrong-approach detector at much smaller scale: $M^{DH}$ has negative eigenvalues at $K = 30$, $T_{\max} = 200$ (3C, 3D). Selberg-class cross-cut on $\chi_3, \chi_4$ confirms direction-selectivity (3C.3, 3D.2). |
