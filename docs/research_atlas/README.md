...
|---|---|---|
| **3: Direct positivity** (Weil / Li) | 3A, 3B, 3C, 3D complete | Small-n Li-positivity does NOT distinguish zeta from Davenport-Heilbronn (3B). The Weil quadratic form via Gram-matrix eigenvalue analysis IS a wrong-approach detector: $M^{DH}$ has negative eigenvalues, witness deepens with basis size (3C, 3D). Selberg-class cross-cut on $\chi_3$ confirms direction-selectivity: $M^{\chi_3}$ is PSD and the witness $c^*$ for D-H gives $W_{\chi_3}(c^*) > 0$ (3C-3). |
