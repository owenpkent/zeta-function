...
| **4: Analytic** (zero-free regions) | 4B, 4D, 4D.2 complete | 1D LP saturates Fejér $\cos(\pi/(n+2))$ (4B). The d-variate generalization: for nonneg trig polys at uniform degree $(N, \ldots, N)$, $\max c_{1, \ldots, 1} = (2\cos(\pi/(N+2)))^d$ — exactly the tensor product of 1D Fejér optima. The LP-optimal coefficient matrix is rank 1, equal to $q_j q_k \cdots$ where $Q$ is the 1D optimum. **The d-variate problem decomposes**; no new auxiliary inequality is found at this LP family. Genuinely new multivariate inequalities require a different LP structure (weighted objectives or coupled constraints). |
...
- The trigonometric inequality method has a hard theoretical ceiling
...
**Why it failed:**
The tools are exhausted. The trigonometric inequalities that drive the classical method (variants of $3 + 4\cos\theta + \cos 2\theta \geq 0$) cannot be improved. Vinogradov's exponential sum method seems to have a structural ceiling at exponent $2/3$. Reaching $\sigma = 1/2$ from $\sigma \approx 1$ requires not just better estimates but entirely different techniques.
