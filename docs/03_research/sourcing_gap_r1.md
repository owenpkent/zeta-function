...
**Update (2026-07-12): the carrier's own spectral data is added to the measured-empty list; the count half of the fourth shape goes structure-cheap up to $O(1)$** (LEARNINGS #169 (parallel line, merged 2026-07-22; IDs renamed e1p->e1r, e1q->e1s), [`e1r`](../../experiments/spectral/e1r_sonin_projector.md) + [`e1s`](../../experiments/spectral/e1s_rank_one_interlacing.md)). e1r tested the one family the #162 probe could not build (the true operator eigenbasis / Sonin-space projector, as the carrier's own spectral data) and measured the S4 slot EMPTY there too: the Hermitian eigenbasis is unitarily equivalent to the standard basis so it cannot collapse in principle, the $D_{\log}$ and E-map channels are full price under the perturbed-log control, and the one rank drop (the Sonin projection) is a spatial-window mirage the adversary confirmed by execution. The S4 corner is now closed for every BUILDABLE family; the SOLE unbuilt variant is the faithful metaplectic self-dual Sonin projector of arXiv:2310.18423. e1s (the count half of the W6 spec, cont. 3 above) splits the fourth shape's residual: the COUNT of eigenvalues below the two-meter edge is structure-cheap up to $O(1)$ (the rigorous content is Weyl interlacing on the Hermitian Weil form $Q$, $\le \mathrm{rank}(P) = 2$; the pole-free count $= \lfloor T/\phi\rfloor$ exactly at $\lambda \le 3$, up to a genuine $O(1)$ at larger $\lambda$ where D-H undercounts the lattice ghost-free), so the fourth shape's residual sharpens to the LOCATION half (where the eigenvalues sit inside that count $=$ M4). Machine floor: the five classical targets are checked ([`lean/ZetaRH/S4Carrier.lean`](../../lean/ZetaRH/S4Carrier.lean), #S4C-1..#S4C-5, 3762 jobs). Nothing here supplies the mechanism; the fourth shape stays a WATCH slot.
