...
- **M-0 (cheap, doable now): the discriminant bridge lemma.** A real binary form $Q(m,n)=m^2 + t\,mn + q\,n^2$ with $Q(m,n)\ge 0$ for all $(m,n)$ has $t^2\le 4q$. Provable immediately by completing the square (it is the positive-definite mirror of #2G-1's `negDef_of_hasseWeil`). This is the bridge from "$\deg\ge 0$" to "$t^2\le 4q$"; it needs no curve theory and can land in `FunctionFieldRH.lean` today. ~1 day.
