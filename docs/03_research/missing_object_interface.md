...
- **B1 rung 5 (the assembly). EXECUTED 2026-08-19** ([`../../experiments/arithmetic_geometric/e2an_sp_object_v0.py`](../../experiments/arithmetic_geometric/e2an_sp_object_v0.py), 27/27; dossier [`e2an_sp_object_v0.md`](../../experiments/arithmetic_geometric/e2an_sp_object_v0.md); LEARNINGS #179). The five components assembled into ONE runnable object at finite scale, K1-clean (Muntz's formula extracts the multiplier from the integer lattice; an oracle counter proves no L-value or zero is consumed). Zeta fills the whole column (emergent spectrum 29/29 to $T = 100$ from integer data; descent identity 3.3e-10; trace formula two-sided to 1.0e-7 on the object's own spectrum; prime-side Weil Gram = zero-side to 2.7e-5 with window margin $-1.9\times10^{-16}$, zero at machine precision); the controls fail componentwise per the bracket (D-H: off-line pair invisible at 600x contrast, $b_6 = 1.936$ Euler leak; Beurling: truncation drift 0.505, duality defect 0.665, residue = its own density). The wrongness localizes at exactly C1 (SP4's residual: the finite shadow of Connes 1998 Theorem 5) and C2 (SP5's margin: uniform-in-scale positivity = M4). The matrix in §3 gains its first BUILT row, and any future candidate drops into the pipeline for a cell-by-cell score. Handed forward: the scaling ladder (EXECUTED same day as rung 6 below), the D-H invisibility certificate, and B2 with this object as the four-of-five inhabitant.
- **B1 rung 6 (the scaling ladder). EXECUTED 2026-08-19** ([`../../experiments/arithmetic_geometric/e2ao_scaling_ladder.py`](../../experiments/arithmetic_geometric/e2ao_scaling_ladder.py), 16/16; dossier [`e2ao_scaling_ladder.md`](../../experiments/arithmetic_geometric/e2ao_scaling_ladder.md); LEARNINGS #180). The SP5 window margin has a CLOSED FORM: $\mathrm{margin}(\sigma) = 4\sqrt{\pi}\,\sigma\,e^{-\gamma_1^2\sigma^2}$ (slope and intercept matched to four figures, $R^2 = 1.000000$, 38 orders of magnitude), with the worst mode the UNMODULATED bump because the explicit formula cancels the pole exactly (the central hole $(-\gamma_1, \gamma_1)$ is the decisive spectral geometry; the registered midgap guess was refuted by the run). The C2 price in the object's coordinates: prime-side certification of the margin at window scale $\sigma$ costs $e^{\gamma_1^2\sigma^2}$ in assembly precision (bottom rung certified to $3.8\times10^{-7}$; floor crossed at $\sigma = 0.3$; saturated beyond). C1 does NOT bind at the tested meters (SP4 residual flat at $10^{-8}$-$10^{-7}$ over prime windows to $e^6$), and the D-H invisibility certificate is scale-robust across carriers $L = 8..16$.
- **B1 rung 7 (the xi arc: the SP5 ladder pushed into contact with the live CCM-Suzuki program). EXECUTED 2026-08-20/21** (LEARNINGS #183-#190; modules e2aq/e2ar/e2as/e2at/e2au/e2av in `experiments/arithmetic_geometric/`; library `references/10_weil_positivity/`). The window Weil-form's ground state measured across scales at 50/80/110 digits with convergence gates and certificates: the hard-window state IS $\Xi$ at $a = 1$ (residual 0.051), NARROWS monotonically after (certified through $a = 4$, no turnaround), the object identified AT SOURCE as the one in Suzuki's conjecture (1.2), and the closing measurement: CCM's kernel is exactly $\Xi$ at every window while the kernel-groundstate proximity (their self-named "main remaining obstacle") decays 0.9988 $\to$ 0.715 past their evidence range. The $\Xi$-shape exists in every window but is not the RESOLVABLE minimizer beyond $a \approx 1$: and the coda (e2aw, LEARNINGS #191) measured the price and inverted the bridge: the kernel's exact-vanishing identity makes its true window energy tail-controlled ($\lg B \approx -2\pi e^{2a}/\ln 10$), undercutting the instrument's certified bottom from $a = 1.5$ (4/92/8060 orders at $a = 1.5/2/4$), so the certified collapse is a resolvable-subspace statement, (1.2) is horizon-limited for every direct-minimization instrument, and nothing measured contradicts CCM's expectation about the continuum object. The arc's transferable law: the season's second horizon law. Packaged as P12 (draft v0.2, 2026-08-21).

- **B1 rung 8 (the shadow column: the F_q instance of the whole pipeline). EXECUTED 2026-08-23** (LEARNINGS #193; [`e2ay_ff_twin`](../../experiments/arithmetic_geometric/e2ay_ff_twin.md), 13/13). The e2an mechanics run on the e2b curve's divisor lattice: every cell EXACT and theorem-backed (extraction $10^{-12}$ via the pole-ARRAY regularizer available from lattice data; duality with no archimedean factor; completeness 51/51 = Hasse's theorem, zeros on exact progressions to $10^{-16}$; SP3 exact; SP4 exact Lefschetz; SP5 inhabited: $|\alpha|^2 = q$ at $10^{-10}$, Hasse margin 1.47, source Hodge index). BOTH open joints close in the shadow: C1 by Frobenius-Lefschetz, C2 by Hodge index: and the per-cell zeta-vs-$\mathbb{F}_q$ table (the dossier's Section 2) displays the missing structure cell by cell: the standing regression control for every future candidate.
