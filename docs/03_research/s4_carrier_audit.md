...
- **Against #152/#153**: the lattice clause reappears as the glue across
  ...
  [AMENDED 2026-07-31, LEARNINGS #172, reading_notes/modular_hecke_sweep_2026-07-30.md
  D1: this bullet previously said "the UNIQUE glue" whose "ONLY known carrier" is the
  lattice. False as written. There are exactly TWO known glues across incommensurable
  circles: the additive lattice, and Lee-Yang stability of a multivariate polynomial
  (Kurasov-Sarnak, J. Math. Phys. 61 (2020) 083501, arXiv:2004.05678 Thm 1, for
  arbitrary reals b_j > 1; classified by Alon-Cohen-Vinzant arXiv:2303.03201 Cor. 1.4).
  The verdict SURVIVES with a better reason: the second glue takes an arbitrary
  multiplicative generator set as input and consumes no lattice, so it is
  Beurling-generic by its own hypotheses and the Section 3 DMV screen kills it BY NAME.
  The S4 spec's clause 4 (lattice-consuming, MANDATORY) is CONFIRMED. Open tension D3
  (Lee-Yang polarity vs the breadth program's demotion) logged, not resolved.]
...
## 8. Addendum 2026-07-12: the Sonin corner CLOSED for buildable families + the Lean floor discharged (e1r + S4Carrier.lean, LEARNINGS #169 (parallel line, merged 2026-07-22; IDs renamed e1p->e1r, e1q->e1s))

The handed-forward Sonin-projector item (Section 7, BUILDER next-executable) is now discharged, and the five VERIFIER targets (Section 7) are machine-checked. Full records: [`experiments/spectral/e1r_sonin_projector.md`](../../experiments/spectral/e1r_sonin_projector.md) (+ `.py` / `.npz`, 10/10) and [`lean/ZetaRH/S4Carrier.lean`](../../lean/ZetaRH/S4Carrier.lean). One ADVERSARY round (e1r PASS_WITH_FIXES, Lean PASS). The frontier is UNMOVED.

- **e1r closes the Sonin item, negative, at buildable scope.** The one candidate family this audit could not test (the true operator eigenbasis / Sonin-space projector) was built as the carrier's OWN spectral data and measured empty. Because Q is Hermitian its full eigenbasis is unitarily equivalent to the standard basis (sv match 3.6e-15) and cannot collapse in principle; the Weil energy eigenbasis, the non-orthogonal $D_{\log}$ eigenbasis, and the E-map shift-sum proxy are all FULL PRICE at $\{\log p\}$ under the perturbed-log control, and the aperiodic $x^{1/2}$ weight is honestly UNBUILDABLE on the compact circle. The ONE rank drop (the Sonin projection) is a SPATIAL-WINDOW MIRAGE, confirmed by the adversary's own three window constructions by execution (empty window -> drop 0; perturbing only the in-window logs -> drop persists exactly; top-k window -> drop tracks $\#$comb-in-W). `s4_spec_answer = NEGATIVE, closed for every buildable family`; the SOLE unbuilt variant is the faithful metaplectic self-dual Sonin projector of arXiv:2310.18423 (the discrete central-window prolate is its finite proxy). The incommensurability reading of Section 5 now extends to the carrier's own eigenbasis: the S4 slot is measured empty in every family that can be built here.
- **The Lean floor is discharged (#S4C-1..#S4C-5).** The five deliberately classical VERIFIER targets landed sorry-free, `#print axioms` exactly `[propext, Classical.choice, Quot.sound]`, full build green 3762 jobs: the prime-side/Euler-gate inequality (corrected #161 nonnegativity form; `vonMangoldt_nonneg` discharges the gate, sign-changing $\Lambda_{DH}$ fails it), the tail-divergence KERNEL (the one classical Chebyshev input $\psi(x) \ge c\,x$ carried as an honest EXTERNAL hypothesis $=$ a named Mathlib gap and upstream-PR candidate), the structural span nil, the decimation rank-1 collapse (the $\mathbb{F}_q$ Frobenius avatar), and trig-Vandermonde full rank at distinct nodes.
- **The count clause of #154's W6 (rank-one interlacing) is separately DONE (e1s, LEARNINGS #169 (parallel line, merged 2026-07-22; IDs renamed e1p->e1r, e1q->e1s)):** the W6 count half is structure-cheap up to $O(1)$ (Weyl-on-$Q$ rigorous, $\le 2$), the location half $=$ M4. The metaplectic self-dual Sonin projector is the one BUILDER residual on this coordinate; the $\theta \le 1/2$ Beurling corner and Carneiro-Littmann well-posedness remain the SURVEYOR residuals; the von Koch/Schoenfeld converse stays the one open citation.
