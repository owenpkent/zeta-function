...
| **The loop driver** | **Built** | `engine.py` + `test_engine.py` (5/5); runs a cycle, prints anti-theater counts, writes a monotone log |
| **Construction-registry handoff** | **Built** | `engine.py` `audit_dh_flags`; recomputes `dh_buildable` and confirms the seed flags |
| **Lean closability hook** | **Built** | `lean_hook.py`; advisory textual audit + authoritative `lake build` path; promotes nothing |

As of 2026-06-13 all five increments are built and green on the existing graph: the executable oracle (1), the rescoped residual layer (2'), the value ranking and asserted-vs-proven reporter (3), the loop driver plus the construction-registry handoff (4), and the Lean closability hook (5). The one thing not on that list is the original four-bit "collision engine": the adversary showed the discrimination it promised already lives, honestly, in the oracle, so it was cut before it cost a line of code. What remains is not more engine but more mathematics: shrinking the gap-16 frontier by proving the annotation edges into M4, which is the arithmetic Hodge standard conjecture and is exactly as hard as it always was.
...
**Increment 4: the loop driver (built 2026-06-13).** `experiments/lemma_db/engine.py` runs one cycle (ingest -> falsify -> route -> score -> report) over a candidate: it calls the oracle, scores the kernel from `frontier_ranked` + `asserted_vs_proven`, prints the anti-theater kill tally, and appends the cycle to a monotone JSONL log under `_cache/`. It carries the construction-registry handoff (`audit_dh_flags`): for nodes with a registered construction it recomputes `dh_buildable` and cross-checks the seed's hand-set flag (today, formation-rule stand-ins confirm `AX-polarization` = false and `CAND-connes-1999-adele-trace` = true). `test_engine.py`: 5/5. `python -m experiments.lemma_db.engine` runs the demo.

**Increment 5: the Lean closability hook (built 2026-06-13).** `experiments/lemma_db/lean_hook.py` is the soundness floor. It **promotes nothing**: the authoritative check is `lake build` of the node's module plus a scan for "declaration uses 'sorry'" warnings (`run_build=True`, needs the toolchain), and the cheap default is a textual file-level sorry scan that is explicitly advisory, because file-level sorry presence does not localize to a declaration (`RHEquivalences.lean` carries sorries yet holds the sorry-free anchor `FND-rh-pi01`). Running it over the 18 `lean_ref` nodes today: 0 dangling refs / overclaims, 10 consistent `proven_lean`, 1 review-flag (the `FND-rh-pi01` file-vs-declaration case), 7 scaffolds. `test_engine.py` covers dangling detection and the consistent case. Promotion to `proven_lean` stays a deliberate human edit informed by an authoritative build.
