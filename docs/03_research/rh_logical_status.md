...
- **(B) Prove the elementary decidable half. PARTIALLY LANDED 2026-06-04.** Sorry-free results in `RHEquivalences.lean` (all with clean `#print axioms`, no `sorryAx`, no `ofReduceBool`): `rh_arith_refutable` (the $\Sigma^0_1$ structure: $\neg$ `RH_arith` $\iff$ a single $n$ violating Lagarias, the formal core of §2's "no false-but-unrefutable"); concrete $\sigma$ computations (`sigma_one_six`, `sigma_one_twelve`) witnessing the matrix's arithmetic side is kernel-computable; and `lagarias_holds_at_three`, the first transcendental instance ($\sigma(3) = 4 \le 11/6 + e^{11/6}\log(11/6) \approx 4.08$), proved by EFFECTIVE BOUNDS on the constants ($e^{11/6} = e\cdot e^{5/6} \ge e\cdot(11/6) \ge (27/10)(11/6)$ via `add_one_le_exp` + `exp_one_gt_d9`, and $\log(11/6) \ge 5/11$ via `log_le_sub_one_of_pos`). That last one concretely realizes the §1 claim that the matrix is decidable by effective approximation, not as a literal `Decidable` real predicate. Note: $n=2$ has too little margin for these crude bounds (it needs sharper ones); proving Lagarias for *all* $n$ would be proving RH. So the open part is more bounded-$n$ instances, not the structure. The analytic forward direction stays deferred (#LG-1).
