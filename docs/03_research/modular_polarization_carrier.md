...
- **MC.2** (done, e2qq, #101): identify $\log\Delta$ with $\Theta$ (#41) and $J$ with $C_E$ (#70) on the finite Euler-Sen model. Result: the identifications fail in instructive ways that **factor** the gap rather than close it. $\Theta$ is non-semisimple ($N\neq0$), so $\log\Delta$ matches only its semisimple part (the weight); the monodromy $N$ is invisible to the modular Hamiltonian (#69). And $C_E^2=-I$ (a complex-linear complex structure) while $J^2=+I$ (antilinear), so $J\neq C_E$: $C_E$ is the polarization phase, $J$ the real-structure conjugation, extra data beyond the modular conjugation. The $C_E$-twisted polarization $B_E$ carries $t$ (PD iff Hasse-Weil) where the modular weight data and any trace form are $t$-blind. So the modular scaffolding is $t$-independent; $C_E$ and $N$ are the $t$-carrying / monodromy data on top. Refines #99: the discriminator is **carries-$t$**, not definite-vs-indefinite (both the trace and $B_E$ are PD).
- **MC.3** (done, e2rr, #102): the finite-prime modular structure is euler-gated. The BC partition function $Z(\beta)=\zeta(\beta)=\prod_p(1-p^{-\beta})^{-1}$ has positive Gibbs weights and a pole at $\beta=1$ (type III$_1$) because $\zeta$'s von Mangoldt comb is non-negative; D-H's comb goes negative (first at $n=3$, $\Lambda_{\mathrm{DH}}(3)=-\kappa\log3$, 21 negatives by $n\le60$), so no positive Gibbs state forms. The archimedean $\Gamma$-factor is shared (both have an FE), so the discriminator is purely the finite-prime comb (#44). The carrier passes K2 by construction (D-H cannot form it), like the oracle's `NoEulerProduct` / Lean `no_dh_cupTarget`.
- **MC.4** (attempted, e2ss, #103; the kernel = M4, stays open). The smallest-case attempt is **K1-circular**, with a sharp mechanism: the BC/KMS carrier lives at $\mathrm{Re}(s)>1$ (absolute convergence of the Euler product), while the local GL$_2$ factor's off-line zeros live in $1/2<\mathrm{Re}(s)<1$ (the range $2\sqrt p<|t_p|<p+1$), so the carrier is **blind** to the very obstruction $t$ encodes; $\log\Delta$ is t-blind, and asserting $|t_p|<2\sqrt p$ per block IS the conclusion. The residual is the **coupling** between the per-prime blocks (the global Frobenius/Lefschetz signed-trace pairing = the product-surface / Poincare-duality assembly) that forces $|t_p|<2\sqrt p$ from the modular flow rather than asserting it. That coupling is M4 undiminished, and per #97 it must be **sourced**, not propagated. So the modular carrier sharpens *where* the wall is (the carrier cannot reach the critical strip) without moving it, exactly as the frame-audit (#98, P1-P5 = NONE) predicted.

The adversary test AT-1 (can the coupling be sourced from the carrier?) is now closed in the negative ([`e2tt`](../../experiments/arithmetic_geometric/e2tt_coupling_attempt.md), #104): the carrier data ($\log\Delta$, the Gibbs weights, any carrier-derived coupling) is t-independent, so it cannot encode the t-dependent constraint; the coupling must be a t-carrying coupling of the $C_E$ polarization phases, i.e. the global cup product. That object has **four unified faces**, all open over $\mathrm{Spec}(\mathbb{Z})$ and all equal to M4: the modular $C_E$-cup (this thread); the function-field cup (Castelnuovo-Severi / 2G, a theorem); the AHK arithmetic lattice ([`research_directions/09A_ahk_arithmetic_lattice.md`](research_directions/09A_ahk_arithmetic_lattice.md)); and the Faltings-Hriljac product $+$ $\Gamma_S$ (2H is the single-surface theorem). The modular, AHK, and product-surface threads are four windows onto the same missing coupling, not separate attacks.

The cheap modular-carrier milestones (MC.1-MC.3) are done; MC.4 is confirmed to BE M4. A Lean scaffold of the function-field RH chain ([`../../lean/ZetaRH/FunctionFieldRH.lean`](../../lean/ZetaRH/FunctionFieldRH.lean), lever B) makes the eigenvalue-extraction link (Hasse-Weil $\Rightarrow|\alpha|=\sqrt q$) sorry-free and isolates the geometric step as the one sorry.
