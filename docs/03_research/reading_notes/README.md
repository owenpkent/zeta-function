...
| [CCM-2021-Prolate-Sonin](CCM-2021-Prolate-Sonin.md) | The Connes-Moscovici prolate paper (2112.05500) as the C3 archimedean-injection object: the prolate/Sonin operator + map E + W_infinity on the real log-line supply the carrier the summed explicit formula S(f) enters through. Section-7 uniformity (of the D_log descendant) = C3 injection-uniformity as object+location identity (MECHANISM), not as logical equivalence (C3 silent on whether the limit holds = M4). Grounds ccm_semilocal_prolate.md + ccm_zeta_cycle_density_gate.md. |
| [CCM-2025-Dlog-family](CCM-2025-Dlog-family.md) | The CCM rank-one D_log family (2511.22755) + its Caratheodory-Fejer self-adjointness engine (2511.23257): the exact finite-cutoff determinant identity det_reg(D_log - z) = -i lambda^{-iz} xihat(z) with real zeros (Thm 5.10(iii)), and the Section-7 uniform limit xihat_lambda -> Xi = "the main remaining obstacle" = M4. Source spec for the e1k D-H testbed (LEARNINGS #158): the CF gate needs only real+even+lower-bounded+simple, no Euler product, so it RUNS on D-H, and finite reality is information-free (D-H twin real to 7.8e-30). |
| [ccm_zeta_cycle_density_gate](ccm_zeta_cycle_density_gate.md) | The CCM zeta-cycle / D_log line vs the e2aj density gate: one circle of length log x carries the whole eigenvalue budget, primes enter as Weil-form coefficients to depth n<=x (the two-meter law); det_reg(D_log - z) = -i lambda^{-iz} xi-hat(z) exact at finite cutoff, open at the uniform Section-7 limit xi-hat_lambda -> Xi. |
