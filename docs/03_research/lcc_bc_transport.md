...
## SYNTHESIZER update (2026-06-13, P2a instrument corrected + function-field wind tunnel)

Two overnight runs (executed 2026-06-12/13) settle the P2a-armability question the stress round left open and stress the rigidity mechanism in the model where RH is a theorem. See [experiments/positivity/e3x3_rung7.py](../../experiments/positivity/e3x3_rung7.py) (LEARNINGS #89) and [experiments/arithmetic_geometric/e2ll2_ff_pinching.py](../../experiments/arithmetic_geometric/e2ll2_ff_pinching.py) (LEARNINGS #88).

- **The c_6 "floor" is an artifact: P2a is NOT LP-armable.** e3x3 reruns the P2a probe with the three confounds the stress round identified removed: the grid-quantization slack $Q \sim L^3 h^2$ is held fixed via $h \sim L^{-3/2}$, the ceiling is extrapolated to zero tolerance and gain-normalized, and comb tests within $\varepsilon$ of the support end are trimmed. The tolerance-free intercept $c6_0$ then FALLS monotonically across the L-ladder ($0.00967 \to 0.00801 \to 0.00802 \to 0.00722 \to 0.00664$ over $L = 8.52 \to 10.09$), with the $c_1$ control falling in lockstep ($0.00260 \to 0.00209$). The apparent $\sim$0.01-0.02 intercept of #84 was still residual; it shrinks with refinement rather than parking on a positive constant. This downgrades P2a from "kill armable" to "kill not LP-armable": the $b_6 = 0$ rigidity is not LP-visible and must be reached as an explicit-formula positivity statement, consistent with #85's "the EF slice is the whole battlefield." Honest scope: the decline is gentle and the run does not certify the $L \to \infty$ limit is exactly 0; it removes the armable-floor reading, not the possibility of a much smaller genuine floor. The signed-family intercept (0.00914) tracks the unsigned, respecting the #84 D-H-blindness rule.
- **The function-field cone forces composite pinching (no ghost).** e2ll2 is the hostile follow-on #79 skipped. Over $\mathbb{F}_3/\mathbb{F}_5/\mathbb{F}_7$ (six curves, genus 1 and 2, where RH is a theorem and the cone face structure is finite and exactly enumerable) it searches for a ghost comb that lies in the Weil positivity cone yet carries von Mangoldt mass at a forbidden composite place (the $\mathbb{F}_q$ analogue of $b_6 = 0$). It returns ZERO ghosts in every case. Composite pinching is forced by the cone, not an extra hypothesis, so LCC + EFR rigidity survives the wind tunnel. Standard caveat (#80): this is the model that already HAS the missing surface and polarization; it does not transfer to Spec(Z), where zeta's moment matrix never goes flat.
- **Live LP targets after these runs:** the $c_2$ shell calibration / P2b $\kappa_2 \to \log 2$ squeeze remains the strongest LP survivor; P2a is now closed to the LP family and handed to the EF-positivity leg; P4 (the unit-atom archimedean obligation) is unchanged.

> Lean status (same overnight run): [CrystalCocycle.lean](../../lean/ZetaRH/CrystalCocycle.lean) gained the V4 expansion. The analytic pair anticipated in the ADVERSARY disposition above (`slope_one_pin`/`slope_one_diverges` = "every crystal has slope 1"; `flat_comb_excluded`/`flat_comb_trough_witness` = "the flat ray is not a crystal") landed sorry-free and axiom-clean (build green, 3146 jobs, axioms `[propext, Classical.choice, Quot.sound]`). The V4 measure-assembly step (B1, LEARNINGS #86) is stated OPEN, not stubbed.
