...
**Deninger (1992+)** — Foliated dynamical systems. Not strictly an $\mathbb{F}_1$ candidate but adjacent: $\mathrm{Spec}(\mathbb{Z})$ should be replaced by a foliated dynamical system $X$ with a real-time flow $\Phi_t$; the leafwise cohomology of this foliation should give the zeta zeros as Frobenius-like eigenvalues, with positivity coming from the Hodge theory of the foliation. **UPDATE 2026-06-02:** the space $X$ is now CONSTRUCTED (Deninger, *Dynamical systems for arithmetic schemes*, arXiv:1807.06400, 2018, via rational Witt spaces; periodic orbits biject with closed points; verified), and the determinant Lefschetz formula is proven for genuine 3-manifold foliated systems (2024). What remains open is the leafwise cohomology's Poincare duality and **polarization** (the Hodge-index signature = RH). See [`2A_deninger_dossier.md`](../../experiments/arithmetic_geometric/2A_deninger_dossier.md) and [`spec_z_cohomology_landscape.md`](spec_z_cohomology_landscape.md). Scorecard: 2 ✅ / 3 🟡 / 12 ⏳ / 0 ❌ — the realization is now built; the polarization is the open content.
