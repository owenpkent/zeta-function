...
Status: 2026-06-13. The governing obstruction (section 1) is the project's own marginal-positivity finding, and it dictates the entire shape of what follows. As of this date the function-field shadow (6d), the move-library generator (6a), and the quality-diversity archive (6c) are built and green ([`fq_shadow.py`](../../experiments/lemma_db/fq_shadow.py), [`generator.py`](../../experiments/lemma_db/generator.py), `test_generator.py` 5/5); transfer-search (6b) and new-branch spec generation (6e) remain design.
...
- **6e: new-branch spec generation.** Read forcing questions off the deepest asserted-vs-proven gaps (the [`asserted_vs_proven`](../../experiments/lemma_db/schema.sql) view), turn each into a falsifiable spec of the missing object, and filter the posited objects by consistency / inhabitation (Lean), realization, the D-H firewall, and $\mathbb{F}_q$ reproduction. Deliverable: a short list of surviving branch-specs, the regime-one (spec-and-fill) part of section 5. The construction of any surviving object stays human (or future-AI). Still design.

Built order so far: 6d, then 6a, then 6c (all 2026-06-13). Remaining: 6e (forcing-questions off the gaps, reuses 6d's filters), then 6b (the prize).
