...
>
> This is a design, not a build. It is written in the honest register of [`proof_program_ai_only.md`](proof_program_ai_only.md): the odds are long, most of it will not work, and the value is in the parts that fail cheaply and the one bridge that might not. Nothing here claims a shortcut to RH.

Status: 2026-06-13. First draft. The governing obstruction (section 1) is the project's own marginal-positivity finding, and it dictates the entire shape of what follows.
...
- **6a: the move-library.** Encode ~7 structure-preserving moves as `Formulation -> [Formulation]`. Run the generated tree through `run_cycle`. Deliverable: the pruning-rate measurement and the first machine-proposed candidates. Cheapest; mostly plumbing over what exists. Testable now (the oracle and the $\mathbb{F}_q$ shadow already run).
- **6b: transfer-search.** Curate a small corpus of proven positivity theorems with hand-tagged structural features; embed M4's residual; retrieve nearest neighbors; LLM judge proposes the functor; oracle + $\mathbb{F}_q$ shadow prune. Deliverable: a ranked list of bridge candidates, the Bost-Connes import automated. The high-value, high-difficulty piece.
- **6c: the quality-diversity archive.** A MAP-Elites grid over the failure-mode axes of section 6, fed by 6a, 6b, and 6e, steering generation toward empty cells. Turns the generators into a loop that maps rather than re-climbs.
- **6d: the $\mathbb{F}_q$-shadow filter as a first-class cheap evaluator.** Promote the function-field specialization check to a standard disqualifier alongside the oracle's, so every generated candidate is filtered by both the negative (D-H) and the positive ($\mathbb{F}_q$) discipline.
- **6e: new-branch spec generation.** Read forcing questions off the deepest asserted-vs-proven gaps (the [`asserted_vs_proven`](../../experiments/lemma_db/schema.sql) view), turn each into a falsifiable spec of the missing object, and filter the posited objects by consistency / inhabitation (Lean), realization, the D-H firewall, and $\mathbb{F}_q$ reproduction. Deliverable: a short list of surviving branch-specs, the regime-one (spec-and-fill) part of section 5. The construction of any surviving object stays human (or future-AI).

Suggested order: 6d first (it is a cheap, high-value evaluator that strengthens everything downstream), then 6a (the cheapest generator), then 6c (the archive), then 6e (forcing-questions off the gaps, reuses 6d's filters), then 6b (the prize).
