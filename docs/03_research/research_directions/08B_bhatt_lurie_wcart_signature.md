...
> **Branch pruning on task (b) (2026-06-05, [`lane2_petrov_nonsemisimplicity.md`](../lane2_petrov_nonsemisimplicity.md)).** Task (b) cannot be done by diagonalizing $\Theta$ into honest Sen eigenspaces and reading positivity weight-by-weight: Petrov ([arXiv:2302.11389](https://arxiv.org/abs/2302.11389), Annals) gives geometric prismatic examples where $\Theta$ is genuinely non-semisimple, and a positive eigenspace HR form built from $\Theta_{\mathrm{ss}}$ alone forces the nilpotent $N = 0$. So semisimplicity is not a free property of WCart geometry; it would have to be proved separately for the $\mathrm{Spec}(\mathbb{Z})$ object, or the positivity must be an $N$-aware monodromy-primitive form $Q_r(x,y) = Q(x, N^r y)$ built BEFORE diagonalization. This is confirmed computationally by 2LN (#69, the naive Lyapunov metric fails on a defective block; the Jacobson-Morozov monodromy quotient survives), 2LO (#70), and 2P+ (#71). The non-semisimplicity is routable, not fatal; the route must carry the full $F + \Theta$ Frobenius geometry, not the Hodge-Tate/Sen fiber alone.