...
The M3 experiment (`e3m_analytic_domination.py`) fundamentally sharpened the trace-side goal of the proof program. We now know that the false positivity of Davenport-Heilbronn (the stealth window) is an artifact of composite prime support ($P_{\mathrm{comp}}$) compensating for a failure in the primitive geometry. 

The true, non-circular positivity statement equivalent to RH is:
$$ M_{\mathrm{euler}} = A_{\mathrm{arch}} + P_{\mathrm{pp}} + B_{\mathrm{pole}} \ge 0 $$
...
4. The composite obstruction $P_{\mathrm{comp}} = 0$ is guaranteed geometrically because the absolute prismatic complex is built prime-by-prime (the $p$-adic completion stricture), meaning cross-prime composite terms naturally vanish in the canonical filtration.

The theoretical task is now to prove that the Sen operator $\Theta$, acting as the connecting map between the Nygaard graded pieces, forces the alternating sum of traces (the Lefschetz trace) to be positive-definite on the primitive primitive cohomology, identically recovering $M_{\mathrm{euler}} \ge 0$.