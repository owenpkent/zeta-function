...
**The Rosati ladder (08A), status 2026-05-30.** The signature target is sharpened to **RH = arithmetic Rosati positivity = the arithmetic Hodge standard conjecture** with the M1-M5 ladder ([`08A_rosati_standard_conjecture.md`](08A_rosati_standard_conjecture.md)). M1 done (function-field Rosati, four equivalent faces, 2T). M2/M2.5/M2.6 done (2U/2V/2W, LEARNINGS #32-34): the non-circular truncated Weil form $M = A_{\mathrm{arch}}+P_{\mathrm{fin}}+B_{\mathrm{pole}}$ is built from the Gamma factor + primes (no zeros), with a validated Bombieri archimedean block ($C_\mu = \log\pi-\psi((1+\mu)/2)$); $\min\mathrm{eig}(M)=+0.035>0$ for $\zeta$ (non-circular positivity, necessary-not-sufficient), but the four-way RH-vs-Euler discrimination FAILS -- D-H reads spuriously positive, the stealth window (#18/#19/#27) on the Rosati side. **CONSEQUENCE: M3 must be ANALYTIC** (the archimedean-dominates-prime domination via the exact off-line structure); finer numerics cannot separate $\zeta$ from D-H. The numerical infrastructure is complete + validated; the analytic domination is the open frontier.
