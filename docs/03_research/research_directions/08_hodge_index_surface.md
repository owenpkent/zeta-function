...
**Sharpened by 2CC (2026-06-02, LEARNINGS #40).** The stated obstacle ("tropical varieties live in $\mathbb{R}^n$, not over $\mathrm{Spec}(\mathbb{Z})$") is *resolved* by the Connes-Consani square: the arithmetic objects DO live as convex polygons in $\mathbb{R}^2$, and the mixed-volume / Alexandrov-Fenchel Hodge index applies there for free. But 2CC shows the resulting tropical Hodge index is **arithmetic-blind**: it freezes the Frobenius trace $t$ (the very thing $|t|<2g\sqrt q$ = RH-for-C needs), so its signature is RH-agnostic. So 4.A's real obstruction is **not** the absence of a tropical model; it is that the tropical/characteristic-1 model's signature loses the trace, and the missing step is the **arithmetic $q$-lift** that restores it (un-idempotent-izing the characteristic-1 operations / building a Weil cohomology with subtraction). The $(1,p)$ bidegree (2Q) is where the lift must inject $t$. This re-scopes 4.A from "find a tropical model" (done, via C-C) to "lift the tropical Hodge index by the arithmetic" (the hard part), and aligns it with attack angle 4.C (the arithmetic refinement).
...
**First probe of the Connes-Consani square (2026-06-02, 2CC, LEARNINGS #40).** [Experiment 2CC](../../../experiments/arithmetic_geometric/e2cc_tropical_shadow.py) (writeup [e2cc_tropical_shadow.md](../../../experiments/arithmetic_geometric/e2cc_tropical_shadow.md)) probes the gap on the actual product object (Connes-Consani's arithmetic-site square = Newton polygons under convex-hull-union + Minkowski-sum). The Minkowski structure carries a canonical mixed-volume form that is Lorentzian by Alexandrov-Fenchel: **a Hodge-index signature `(1,k)` exists for free.** BUT it is the characteristic-1 SHADOW: arithmetic-blind, it loses the Frobenius trace `t` (its `Delta.Gamma` is the combinatorial `p-1`, not the point count `q+1-t`), so its `(1,3)` signature is UNCONDITIONAL/RH-agnostic. This reconciles with the gap's "no signed pairing": the RH-agnostic convex-geometry shadow exists; the ARITHMETIC pairing carrying `t` does not. **The gap sharpens to "the arithmetic q-lift that restores the trace t," with the (1,p) bidegree (2Q) the locus where the lift must inject it.** Honest: a PROPOSED shadow (toric-BKK analogy), not a constructed intersection theory of the topos; ADVERSARY-checked (PASS, softened).
