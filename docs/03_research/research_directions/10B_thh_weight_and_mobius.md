...
>
> **REORIENTATION (2026-05-30 literature survey, LEARNINGS #29).** The framing below is partly superseded. The proven template is **Hesselholt's** $\zeta(X,s) = \det_\infty(s-\Theta\mid \mathrm{TP}_{\mathrm{odd}})/\det_\infty(s-\Theta\mid \mathrm{TP}_{\mathrm{ev}})$ over $\mathbb{F}_q$: the right object is the flow generator $\Theta$ on the $S^1$-Tate periodic $\mathrm{TP}$ with $\det_\infty$, NOT raw THH orders with a hand-imposed $i^{-s}$ weight, and NOT a Möbius reduction. The $i^{-s}$ power should come from $\Theta$'s eigenvalues (over $\mathbb{F}_q$, $q^\Theta = \mathrm{Fr}^*$ gives $q^{-s}$). **Gap B is flagged as a likely category error** by the adversary: the equalizer $\mathrm{TC}=\mathrm{eq}(\mathrm{TC}^-\rightrightarrows\mathrm{TP})$ is an additive limit, $1/\zeta$ is multiplicative over primes; the necklace/$\mu$ result below is real combinatorics but does not by itself make the equalizer multiplicative. Read Gaps A and B below as the project's first (partly off-target) attempt; the corrected target is the Hesselholt-over-$\mathbb{Z}$ determinant (see [`10A`](10A_thh_vonmangoldt_checkpoint.md) §0).
