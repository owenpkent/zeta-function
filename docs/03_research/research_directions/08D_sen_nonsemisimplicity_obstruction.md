...
> **Lean verification (2026-06-05)**: the compact-group obstruction of section 2 is now machine-checked, sorry-free, against Mathlib in [`lean/ZetaRH/SenDefiniteObstruction.lean`](../../../lean/ZetaRH/SenDefiniteObstruction.lean) (5 theorems; `#print axioms` reports only the three standard Mathlib axioms, no `sorryAx`). The core `skew_nilpotent_eq_zero` (a real skew-symmetric nilpotent matrix is zero, by the elementary trace identity $\mathrm{tr}(N^{\mathsf T}N) = -\mathrm{tr}(N^2) = 0$, no spectral theorem) and the Sen corollaries `sen_no_nilpotent_part` / `sen_nilpotent_part_not_isNilpotent` give the standard-frame no-go. The result is then made **unconditional in the cup form**: `bskew_nilpotent_eq_zero_of_gram` proves the obstruction for any Gram factor $B = S^{\mathsf T}S$ with $S$ invertible, and `bskew_nilpotent_eq_zero` discharges the factor for every positive-definite $B$ via the continuous-functional-calculus square root ($S = $ `CFC.sqrt` $B$). So the $(\star)$ no-go ($\nu \ne 0 \Rightarrow$ no positive-definite invariant cup form) holds for an arbitrary definite $B$, not merely in the orthonormal frame. VERIFIER target #ClassA-1.
