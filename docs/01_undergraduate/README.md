...
The Riemann zeta function is defined for real $s > 1$ by the **Dirichlet series**:

$$\zeta(s) = \sum_{n=1}^{\infty} \frac{1}{n^s} = \frac{1}{1^s} + \frac{1}{2^s} + \frac{1}{3^s} + \cdots$$

**Convergence:** This series converges absolutely for $\text{Re}(s) > 1$. For $s = 1$, it becomes the harmonic series $\sum 1/n$, which diverges. For $s \leq 1$ (real), the series diverges.

### Special Values (real $s$)

| $s$ | $\zeta(s)$ | Notes |
|-----|-----------|-------|
| 2 | $\pi^2/6 \approx 1.6449$ | Basel problem (Euler, 1734) |
| 4 | $\pi^4/90 \approx 1.0823$ | |
| 6 | $\pi^6/945$ | |
| Even integers | $\zeta(2n) = \frac{(-1)^{n+1} B_{2n} (2\pi)^{2n}}{2(2n)!}$ | $B_{2n}$ = Bernoulli numbers |
| Odd integers ≥ 3 | Unknown closed form | $\zeta(3) \approx 1.202$ (Apéry's constant) |

---

## 2. The Euler Product Formula

Euler discovered a stunning connection between the zeta function and the prime numbers:

$$\zeta(s) = \prod_{p \text{ prime}} \frac{1}{1 - p^{-s}} = \frac{1}{1-2^{-s}} \cdot \frac{1}{1-3^{-s}} \cdot \frac{1}{1-5^{-s}} \cdots$$

**Why this works:** Each factor $\frac{1}{1-p^{-s}} = 1 + p^{-s} + p^{-2s} + \cdots$ is a geometric series. The fundamental theorem of arithmetic (unique prime factorization) guarantees that multiplying all these together reproduces every term $n^{-s}$ exactly once.
...
The series $\sum n^{-s}$ only converges for $\text{Re}(s) > 1$. But there is a unique way to **extend** $\zeta(s)$ to (almost) the entire complex plane while preserving differentiability (analyticity). This is called **analytic continuation**.
...
**The result:** $\zeta(s)$ extends to all of $\mathbb{C}$ except for a **simple pole at $s = 1$** with residue 1.
...
Another approach uses the **eta function** (alternating Dirichlet series):
...
The fully continued zeta function satisfies a beautiful symmetry called the **functional equation**:

$$\zeta(s) = 2^s \pi^{s-1} \sin\!\left(\frac{\pi s}{2}\right) \Gamma(1-s)\, \zeta(1-s)$$

Or equivalently, defining the **completed zeta function**:
...
We know:
- $\zeta(s) \neq 0$ on the line $\text{Re}(s) = 1$ (proved by Hadamard and de la Vallée Poussin in 1896 — this was enough to prove the Prime Number Theorem)
...
**The Riemann Hypothesis:** Every non-trivial zero $\rho$ satisfies $\text{Re}(\rho) = 1/2$.

---

## 7. Connection to Prime Counting

Define $\pi(x)$ = number of primes $\leq x$ (the prime counting function).
...
where $\text{Li}(x) = \int_2^x \frac{dt}{\ln t}$ and the sum is over non-trivial zeros.
